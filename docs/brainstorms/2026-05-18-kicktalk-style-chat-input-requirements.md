...
- **7TV personal emote sets.** KickTalk fetches and renders a "Personal" sub-section in its 7TV dialog header. StreamFusion does not currently plumb personal sets (`grep` for `personalEmoteSets` in `apps/desktop/src` returns nothing). Adding personal-set fetching is a follow-up.
...
- **Mobile / touch layout adjustments.** StreamFusion is desktop Electron; Enter-to-send and hover-tooltip are appropriate.
...
- Kick chatroom-info shape matches what KickTalk's `InfoBar.jsx` reads: `chatroomInfo.followers_mode.{enabled, min_duration}`, `subscribers_mode.enabled`, `slow_mode.{enabled, message_interval}`, `emotes_mode.enabled`, `account_age.{enabled, min_duration}`. Planning to verify the StreamFusion API client exposes this surface, or add it.
- KickTalk's `useAccessibleKickEmotes` hook abstracts Kick's per-tier subscriber-only filtering. StreamFusion's existing `useEmoteStore.getEmotesByProvider("kick")` may need a parallel filter layer to support R9; planning decision on whether to extend the store or layer it in the dialog component.
...
- A moderator toggling slow mode in the StreamFusion mod strip continues to see immediate banner update (no regression from optimistic path).
...
- Dialog positioning: anchored popover (KickTalk's pattern, opens above-and-aligned-to button) vs. input-width sheet (StreamFusion's current `EmotePicker` shape)?
...
- Whether to copy KickTalk SCSS class names (`emoteDialog`, `dialogBody`, `dialogBodySection`) directly into a new stylesheet or translate to Tailwind utility classes inline (current StreamFusion convention).
...
- StreamFusion ChatInput today: `apps/desktop/src/components/chat/ChatInput.tsx`
- StreamFusion EmotePicker today: `apps/desktop/src/components/chat/EmotePicker.tsx`
- StreamFusion room-state store (banner data source): `apps/desktop/src/store/room-state-store.ts`
- StreamFusion emote store: `apps/desktop/src/store/emote-store.ts`
