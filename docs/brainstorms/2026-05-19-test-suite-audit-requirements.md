...
- AE1. **Covers R4.** Given a test file `apps/desktop/tests/components/ui/button.test.tsx` that asserts only "button renders without crashing" and "onClick prop is forwarded", when the auditor reviews it, the verdict is **Delete** — these assertions guard no observable StreamFusion-specific behavior (the underlying button library covers them and a real button regression in our app would not be caught). The deletion is included in the `components/` batch commit.
