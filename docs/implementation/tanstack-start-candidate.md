...
The targeted verifier passed every native check on arm64 at `ed9ada655b47a5148fcb42475c1080c9dcecfa11`. Its Home and Settings screenshots, normal quit, database integrity, FFmpeg and logs passed. X64 passed the access probe and rendered live Home video/chat, but its larger accessibility tree still exceeded the scan timeout. The runners had different display sizes, 1024 by 768 and 1920 by 1080. The verifier now sets and verifies a 1024 by 720 native window on both architectures. This makes the shell/navigation check repeatable; it does not establish wider-layout or sustained-chat performance parity. The next x64 result remains pending.
