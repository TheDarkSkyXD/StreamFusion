...
The manual `start-candidate.yml` workflow builds unsigned macOS x64 and arm64 candidates and checks copied packages outside the checkout. The verifier records native accessibility navigation, screenshots, SQLite integrity, FFmpeg execution, logs and artifact hashes. Both architectures passed artifact, FFmpeg and launch checks. Arm64 passed every native check at `ed9ada655b47a5148fcb42475c1080c9dcecfa11` and again at `0ac0d496816db97ac4335b4db830631aca8fef5c`, including Home, Settings, normal quit and database integrity.

X64's AppleEvent traversal timed out despite a successful access probe, clean logs and a screenshot showing Home with live video. Matching both windows to 1024 by 768 did not resolve the failure, which falsified the layout-workload hypothesis. The verifier now compiles a PID-scoped Swift accessibility reader before launching the app. It preserves native Settings activation, bounded traversal, screenshots and package integrity checks. It fails without requesting permission if the runner does not trust the reader. Both architectures must validate this adapter; the x64 navigation gate remains unmet. The append-only decision trail retains each experiment and its result.
