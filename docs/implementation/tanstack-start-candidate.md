...
The responsive change passed 7,787 tests across 649 files locally, type checking, lint and the Start production build. Repository CI for `c9f29e56b5757647d077f56c000ee7d3c9e3ea1f` passed all required jobs, including 7,788 desktop tests, mobile verification and platform builds. The committed Start build also passed local-file reload with zero runtime diagnostics. Earlier CI failures were repaired in mobile architecture checks, scrubber measurements, disabled connectivity state, the native verifier's Node test environment and isolated deferred-preload mocks.

The manual `start-candidate.yml` workflow builds unsigned macOS x64 and arm64 candidates and checks copied packages outside the checkout. The verifier records native accessibility navigation, screenshots, SQLite integrity, FFmpeg execution, logs and artifact hashes. Its Swift reader activates and sizes the owned window, records its window ID and captures that window. Settings readiness requires the active Settings link and loaded panel content. It does not request new permissions or enable packaged remote debugging.

Both architectures passed the automated native checks at `3dc051a` in [run 36253097342](https://github.com/TheDarkSkyXD/StreamFusion/actions/runs/36253097342). The x64 Settings screenshot shows the loaded panel. Replacing repeated AppleEvent traversal with a native reader resolved that run's navigation timeout. Matching window sizes alone had not resolved it. Repository CI also passed all required verification jobs on that commit.

Arm64 visual readiness remains unproven. At `819fafd`, its exact-window capture still shows Home while accessibility reports Settings as the current page with loaded controls. A one-second capture delay and the stronger active-route assertion did not resolve this disagreement. The automated pass proves the recorded accessibility, package and database checks; it does not prove the screenshot content. Evidence is retained in `.scratch/start-migration/macos-arm64-eleventh/_temp/start-candidate-evidence`. Keep the visual adoption gate open and investigate the renderer/capture disagreement separately. The append-only decision trail retains the earlier experiments.
...
- Retain reproducible exact-artifact Windows UI, SQLite, FFmpeg and sandboxed isolated-player evidence independently of repository dependencies. Resolve the arm64 disagreement between native accessibility and captured pixels. Automated macOS package checks have passed on both architectures. Preserve the Linux target.
