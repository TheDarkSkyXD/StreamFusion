...
`channel-follow` is an assigned control. It follows PRD 18.3: account writes require an approved official path; otherwise the UI offers Guest Follow and an explicit provider-page action. `open-channel-about` is a read-only route or disclosure action. The prototype's Follow and Following labels do not prove provider mutation support.
...
`activity` renders a title row with unread count, Mark read, and Clear completed Activity, Activity tabs, then typed Activity rows. A row includes stable event identity, visual, title, timestamp, source or Platform, read state, and a current safe route. All combines channel, job, and system events. Channels and Jobs filter the same durable inbox. A completed item detail exposes Dismiss from Activity and keeps its destination action available while dismissal confirmation is pending; cancelling retains that detail context. Clear completed Activity captures the completed items currently in the durable inbox, confirms that it hides them locally across all Activity tabs even when started from a filtered tab, and reports the persisted result. It does not cancel a job, change media, follows, History, accounts, settings, or Android notifications. Active jobs remain visible until terminal reconciliation and explain why dismissal is unavailable.

`activity-mark-read`, <!-- contract:action:activity-dismiss-item -->`activity-dismiss-item`, and <!-- contract:action:activity-clear-completed -->`activity-clear-completed` are assigned controls. Opening an item may mark that item read after its route resolves. It does not reset other read state. A duplicate completed event preserves local read and dismissal metadata; an active job update overrides prior dismissal visibility without resetting read state. An ended stream opens ended Channel or detail state. A malformed, stale, dismissed, or pruned destination fails safely. The executable prototype responds to its fixture Mark read, per-item Dismiss, and Clear completed Activity controls, but does not prove native persistence.
