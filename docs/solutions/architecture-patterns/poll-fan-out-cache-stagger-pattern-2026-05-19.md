...
- `docs/solutions/conventions/polled-stub-empty-return-no-warn-2026-05-21.md` — stub-side complement to this pattern. The `AbortError` warn-filter in §3 here is a specific instance of the broader "expected states should not warn" principle that convention generalises for `IPlatformClient` stubs whose empty return is the intended contract.
