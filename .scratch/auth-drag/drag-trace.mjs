...
try {
  await call('Page.bringToFront'); await call('Runtime.enable'); await call('Performance.enable');
  await frame();
  const before = await capture('before'); const metricBefore=await call('Performance.getMetrics'); await evaluate(`(()=>{window.__dragFrames=[];let previous;const tick=t=>{if(previous!==undefined)window.__dragFrames.push(t-previous);previous=t;window.__dragRaf=requestAnimationFrame(tick)};window.__dragRaf=requestAnimationFrame(tick)})()`);
  const identities = [];
  ...
  if(action === 'drag') {
    ...
    for(let step=0;step<120;step++){await call('Input.dispatchMouseEvent',{type:'mouseMoved',button:'left',buttons:1,x:point.x+step%16,y:point.y+step%8});} await frame(); await capture('preview');
    await call('Input.dispatchMouseEvent',{type:'mouseReleased',button:'left',buttons:0,clickCount:1,...point});
  } else if(action === 'click') {
    const r=await box(args[0]); const x=r.x+r.width/2,y=r.y+r.height/2;
    await call('Input.dispatchMouseEvent',{type:'mouseMoved',x,y});
    await call('Input.dispatchMouseEvent',{type:'mousePressed',button:'left',buttons:1,clickCount:1,x,y});
  ...
  const metricAfter=await call('Performance.getMetrics'); const metrics=Object.fromEntries(metricAfter.metrics.map(m=>[m.name,m.value-(metricBefore.metrics.find(b=>b.name===m.name)?.value??0)])); const frameSample=await evaluate(`(()=>{cancelAnimationFrame(window.__dragRaf);const s=window.__dragFrames.sort((a,b)=>a-b);return{count:s.length,p95:s[Math.floor(s.length*.95)],max:s.at(-1)}})()`); const result={metrics,frameSample,action,args,before:before.widgets,after:after.widgets,splitters:after.splitters,stable,errors,horizontalOverflow:after.horizontalOverflow};
