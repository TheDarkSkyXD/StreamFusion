...
describe("Media Job workflow", () => {
  it("starts a fixture job, projects Activity, and keeps readAt on recover", async () => {
    ...
    const activity: ActivityRepository = {
      ...
      async record(item) {
        const existing = activityItems.find((candidate) => candidate.eventId === item.eventId);
        if (existing && existing.kind === "job" && item.kind === "job") {
          const next = { ...item, readAt: existing.readAt, occurredAt: existing.occurredAt };
    ...
    const native = {
      readiness: () => ({ capability: "media-jobs", contractVersion: 2, kind: "ready" }),
      ...
      cancelRecoverableJob: async () => ({ kind: "completed", value: { kind: "missing", jobId: "x" } }),
      pauseRecoverableJob: async () => ({ kind: "completed", value: { kind: "missing", jobId: "x" } }),
      resumeRecoverableJob: async () => ({ kind: "completed", value: { kind: "missing", jobId: "x" } }),
      retryRecoverableJob: async () => ({ kind: "completed", value: { kind: "missing", jobId: "x" } }),
      finalizeRecoverableJob: async () => ({ kind: "completed", value: { kind: "missing", jobId: "x" } }),
      getRecoverableJob: async () => ({ kind: "completed", value: { kind: "missing", jobId: "x" } }),
  ...
  it("recovers each stored job when the native list result is unusable", async () => {
    ...
    const native = {
      readiness: () => ({ capability: "media-jobs", contractVersion: 2, kind: "ready" }),
      startRecoverableJob: async () => ({ kind: "unavailable", failure: { code: "NATIVE_RESULT_INVALID", diagnostic: "bad" } }),
      recoverJobs: async () => ({
        kind: "unavailable",
