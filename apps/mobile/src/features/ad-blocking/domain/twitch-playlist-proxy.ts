...
export function isTwitchPlaylistProxyMode(
  preferences:
    | { readonly twitchPlaylistProxy: TwitchPlaylistProxyPreferences }
    | null
    | undefined,
): boolean {
...
/** Resolve enabled template URLs in fallback order for live Twitch Watch. */
export function resolveTwitchPlaylistProxyAttemptUrls(
  preferences: TwitchPlaylistProxyPreferences | null | undefined,
  channelName: string,
): readonly string[] {
  if (!preferences?.enabled) return [];
  const urls: string[] = [];
  for (const source of getEnabledTwitchPlaylistProxySources(preferences.sources)) {
    const resolved = resolveTwitchPlaylistProxyUrl(source, channelName);
