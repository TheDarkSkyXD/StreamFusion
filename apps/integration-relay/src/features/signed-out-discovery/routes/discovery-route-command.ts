...
const ROUTE_KINDS: Readonly<Record<string, DiscoveryRouteKind>> = {
  ...
  "/v1/discovery/channel": "channel",
  "/v1/discovery/channel-videos": "channel-videos",
  "/v1/discovery/channel-clips": "channel-clips",
...
export function commandFrom(
  ...
  if (
    ...
    return channelCommand(kind, platform, url);
  }
  return mediaCommand(kind, platform, url);
}

function channelCommand(
  kind: "channel" | "channel-videos" | "channel-clips",
  platform: DiscoveryPlatform,
  url: URL
): DiscoveryRead | null {
