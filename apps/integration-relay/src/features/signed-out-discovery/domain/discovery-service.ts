...
async function dispatchRead(
  ...
  if (command.kind === "channel") {
    return wrap(command.kind, await catalog.channel(command.lookup));
  }
  if (command.kind === "channel-videos") {
    return wrap(command.kind, await catalog.videos(command.lookup));
  }
  if (command.kind === "channel-clips") {
    return wrap(command.kind, await catalog.clips(command.lookup));
  }
