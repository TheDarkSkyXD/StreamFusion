...
describe("ProxiedImage", () => {
  ...
  it("routes proxied images through the same-origin relay in browser development", async () => {
    ...
    try {
      ...
      expect(img.getAttribute("src")).toMatch(/^\/__streamfusion-dev\/media\?.*kind=kick-image/);
    } finally {
  ...
  it("routes Kick video thumbnails through the kick-image protocol without first requesting the raw 720.webp URL", async () => {
    const upstream = "https://images.kick.com/video_thumbnails/z7oMLoDcD3va/iBP8BzqJxpzh/720.webp";
  ...
  it("leaves non-profile Twitch CDN URLs (emotes, thumbnails) alone", async () => {
    const emote = "https://static-cdn.jtvnw.net/emoticons/v2/25/default/dark/3.0";
  ...
  it("retries a failed Kick proxy image in place instead of latching it for the session", async () => {
    vi.useFakeTimers();
    try {
      const upstream = "https://images.kick.com/video_thumbnails/channel/video/720.webp";
      render(<ProxiedImage src={upstream} alt="Kick VOD retry" />);
      const firstImage = screen.getByRole("img", { name: "Kick VOD retry" });

      Object.defineProperty(firstImage, "naturalWidth", { configurable: true, value: 1 });
      Object.defineProperty(firstImage, "naturalHeight", { configurable: true, value: 1 });
      fireEvent.load(firstImage);
      expect(screen.queryByRole("img", { name: "Kick VOD retry" })).toBeNull();
  ...
  it("skips the network request and shows the fallback when a URL has already 403d this session", async () => {
    ...
    const { unmount } = render(<ProxiedImage src="https://cdn.example.com/broken.jpg" alt="Bob" />);
