...
vi.mock("@/components/chat/InfoBanner", () => ({
  InfoBanner: (props: { platform: string; channelId: string }) => infoBannerImpl(props) ?? null,
}));
...
// Guards: settings gear and Chat submit stay below the textarea in a second row without moving emote pickers down.
// Guards: only the textarea/emote row is outlined; footer actions stay outside the input box outline.
...
describe("ChatInput — footer actions", () => {
  ...
  it("places the settings gear and Chat button below the input row", () => {
    ...
    expect(textRow).toHaveClass("border", "rounded-md", "bg-[#191919]");
    expect(actionRow).toHaveClass("relative");
    expect(actionRow).not.toHaveClass("border-t");
    expect(textRow).toContainElement(screen.getByPlaceholderText(/send a message/i));
