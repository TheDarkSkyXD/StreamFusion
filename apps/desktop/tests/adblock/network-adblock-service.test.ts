  ...
  describe("shouldBlock", () => {
    ...
    describe("Third-party Ad SDKs", () => {
      it("should block Google IMA SDK", () => {
        const result = networkAdBlockService.shouldBlock(
      ...
      it("should block secure DoubleClick ad requests from Kick pages", () => {
        const result = networkAdBlockService.shouldBlock(
      ...
      it("should block Google Tag Manager requests from Kick pages", () => {
        const result = networkAdBlockService.shouldBlock(
      ...
      it("should block Insider analytics requests from Kick pages", () => {
        const result = networkAdBlockService.shouldBlock(
          "https://segment.api.useinsider.com/track"
        );
        expect(result.blocked).toBe(true);
        expect(result.rule?.category).toBe("tracking");
      });
    ...
    describe("Event Tracking", () => {
      it("should block client-event-reporter.twitch.tv", () => {
        const result = networkAdBlockService.shouldBlock(
          "https://client-event-reporter.twitch.tv/report"
        );
        expect(result.blocked).toBe(true);
        expect(result.rule?.category).toBe("tracking");
      });
    ...
    describe("Allowed URLs", () => {
      ...
      it("should allow Twitch video segments", () => {
        const result = networkAdBlockService.shouldBlock(
          "https://video-edge-abc.twitch.tv/v1/segment/123.ts"
        );
        expect(result.blocked).toBe(false);
      });
      ...
      it("should allow usher.ttvnw.net", () => {
        const result = networkAdBlockService.shouldBlock(
          "https://usher.ttvnw.net/api/channel/hls/streamer.m3u8"
        );
        expect(result.blocked).toBe(false);
      });
