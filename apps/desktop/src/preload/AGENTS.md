...
- **Single export name**: the bridge is always `window.electronAPI`. The
  ...
  in `shared/electron.d.ts` to type `window.electronAPI` globally.
...
| `apps/desktop/src/shared/electron.d.ts` | Global `window.electronAPI: ElectronAPI` declaration consumed by the renderer |
