...
| `settings/`     | `ChatSettingsSection` — settings UI for chat display preferences (appearance, emotes, events, behavior). Exports reusable row components; the shared writer hook lives in `features/settings/data/use-chat-display.ts`.                                                                                               |
