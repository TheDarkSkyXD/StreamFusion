...
import { setNetworkStatusOverrideForDebug } from "@/features/settings/components/hooks/useNetworkStatus";
...
function NetworkState({ isOnline, children }: { isOnline: boolean; children: ReactNode }) {
  useEffect(() => {
    setNetworkStatusOverrideForDebug(isOnline);
    return () => setNetworkStatusOverrideForDebug(null);
  }, [isOnline]);
  return children;
}

function withNetworkState(isOnline: boolean): Decorator {
  return (Story) => (
...
const meta = {
  ...
  decorators: [withAppRouter, withShellState(), withNetworkState(true)],
  args: {
...
export const Offline: Story = {
  decorators: [withNetworkState(false)],
};
