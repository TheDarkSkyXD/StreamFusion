...
import { STREAM_KEYS } from "../../../discovery/components/hooks/queries/useStreams";
...
import { getAuthRuntime } from "@/features/auth/composition/auth-runtime";
...
export const useAuthStore = create<AuthState>()((set, get) => ({
  ...
  initializeAuth: () => {
    ...
    const initialization = (async () => {
      try {
        ...
        if (!runtime) throw new Error("Authentication runtime unavailable");
        let status: AuthStatus = await runtime.auth.getStatus();
        ...
        if (status.twitch.hasToken && status.twitch.isExpired) {
          ...
          try {
            const refreshResult = await getAuthRuntime()!.auth.refreshTwitchToken();
            if (refreshResult.success) {
              logger.debug("Store:Auth", "twitch token refreshed successfully");
              // Re-fetch status after refresh
              status = await getAuthRuntime()!.auth.getStatus();
            } else {
        ...
        if (status.kick.hasToken && status.kick.isExpired) {
          ...
          try {
            const refreshResult = await getAuthRuntime()!.auth.refreshKickToken();
            if (refreshResult.success) {
        ...
        // Load local follows
        const follows = await getAuthRuntime()!.follows.getAll();

        // Load preferences
        const preferences = await getAuthRuntime()!.preferences.get();
        ...
        getAuthRuntime()!.auth.onKickSessionExpired(() => {
        ...
        getAuthRuntime()!.auth.onFollowsSynced(({ platform, addedCount, removedCount }) => {
        ...
        getAuthRuntime()!.auth.onTwitchAuthLost(() => {
          logger.warn(
            "Store:Auth",
        ...
        const syncStartupFollows = async (platform: Platform): Promise<void> => {
          try {
            await getAuthRuntime()!.auth.syncFollows(platform);
          } catch (error) {
  ...
  loginTwitch: async () => {
    ...
    const stopDeviceCodeStatus = getAuthRuntime()!.auth.onDeviceCodeStatus(({ status }) => {
      if (status === "pending") {
    ...
    try {
      ...
      await getAuthRuntime()!.auth.openTwitchLogin();
      set({ twitchAuthPhase: "finishing" });
      ...
      if (get().twitchConnected) {
        void queryClient.prefetchQuery({
          ...
          queryFn: async () => {
            const response = await getAuthRuntime()!.streams.getFollowed({});
            if (!response.success) {
  ...
  logoutTwitch: async () => {
    ...
    try {
      ...
      const result = await getAuthRuntime()!.auth.logoutTwitch();
      if (!result.success) {
  ...
  loginKick: async () => {
    ...
    try {
      await getAuthRuntime()!.auth.openKickLogin();
  ...
  logoutKick: async () => {
    ...
    try {
      ...
      await getAuthRuntime()!.auth.logoutKick();
  ...
  refreshAuthStatus: async () => {
    try {
      const status: AuthStatus = await getAuthRuntime()!.auth.getStatus();
  ...
  addFollow: async (follow) => {
    try {
      const newFollow = await getAuthRuntime()!.follows.add(follow);
      set((state) => ({
  ...
  removeFollow: async (id) => {
    try {
      const removed = await getAuthRuntime()!.follows.remove(id);
      if (removed) {
        set((state) => ({
  ...
  updateFollow: async (id, updates) => {
    try {
      const updated = await getAuthRuntime()!.follows.update(id, updates);
      if (updated) {
        set((state) => ({
  ...
  isFollowing: async (platform, channelId) => {
    try {
      return await getAuthRuntime()!.follows.isFollowing(platform, channelId);
    } catch (error) {
    ...
    const sync = (async (): Promise<FollowSyncResult> => {
      ...
      const outcomes = await Promise.all(
        platforms.map(async (platform) => {
          try {
            const result = await getAuthRuntime()!.auth.syncFollows(platform);
  ...
  updatePreferences: async (updates) => {
    try {
      const updated = await getAuthRuntime()!.preferences.update(updates);
