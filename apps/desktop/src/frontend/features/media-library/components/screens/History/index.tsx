...
import { useHistoryActions, useHistoryQuery } from "@/features/media-library/components/hooks/useHistoryQuery";
import { getMediaPlaybackReader } from "@/features/media-library/composition/media-playback-reader";
import { resolveProxiedImageSrc } from "@/lib/proxied-image-url";
import type { HistoryItem } from "@/features/media-library/components/state/history-store";
...
export function HistoryPage() {
  ...
  const findRealClipMetadata = async (item: HistoryItem): Promise<VideoOrClip | null> => {
    const mediaReader = getMediaPlaybackReader();
    if (!mediaReader || !item.channelName) return null;

    let result;
    try {
      result = await mediaReader.getChannelClips({
        platform: item.platform,
  ...
  const verifyVideo = async (item: HistoryItem) => {
    ...
    const mediaReader = getMediaPlaybackReader();
    if (!mediaReader) return true;

    const result = await mediaReader.getVideoPlaybackUrl({
      platform: item.platform,
  ...
  const watchHistoryItem = async (item: HistoryItem) => {
    ...
    try {
      ...
      const result = await getMediaPlaybackReader()?.getClipPlaybackUrl({
        platform: item.platform,
