...
vi.mock("@/features/discovery/components/hooks/queries/useFollowedContent", () => ({
...
vi.mock("@/features/discovery/components/state/follow-store", () => ({
  useFollowStore: <T,>(
...
import { useTopCategories } from "@/features/discovery/components/hooks/queries/useCategories";
import { useChannelByUsername } from "@/features/discovery/components/hooks/queries/useChannels";
import {
  useFollowedClipPlayback,
  useFollowedClips,
  useFollowedVideos,
} from "@/features/discovery/components/hooks/queries/useFollowedContent";
import {
  createFollowedStreamSnapshotIdentity,
  useFollowedStreams,
} from "@/features/discovery/components/hooks/queries/useStreams";
import { FollowingPage } from "@/features/discovery/components/screens/Following";
...
describe("FollowingPage", () => {
  ...
  it("loads followed categories and warms thumbnails when the Categories tab opens", async () => {
    ...
    const { unmount } = renderWithProviders(<FollowingPage />);
    ...
    expect(
      images.every((image) => image.removeAttribute.mock.calls.some(([name]) => name === "src"))
    ).toBe(true);
  });
