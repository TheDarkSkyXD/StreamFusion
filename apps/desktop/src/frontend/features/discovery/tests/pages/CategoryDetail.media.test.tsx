...
vi.mock("@/features/discovery/components/hooks/queries/useCategories", () => ({
  useCategoryById: vi.fn(),
  useInfiniteTopCategories: vi.fn(),
}));

vi.mock("@/features/discovery/components/hooks/queries/useInfiniteStreams", () => ({
...
} from "@/features/discovery/components/hooks/queries/useCategories";
import { useInfiniteStreamsByCategory } from "@/features/discovery/components/hooks/queries/useInfiniteStreams";
import { CategoryDetailPage } from "@/features/discovery/components/screens/CategoryDetail";
...
// Guards: Category media tabs keep the supported Language and View-sort filters visible without offering unsupported Tag filtering
...
// Guards: the Clips time-range, Language, and View-sort controls share one Category filters row
...
describe("CategoryDetailPage media tabs", () => {
  ...
  // Guards: provider errors cannot simultaneously claim that a category has no media.
  it.each(["clips", "videos"] as const)(
    "distinguishes failed %s from a successful empty feed",
    async (kind) => {
      searchState.tab = kind;
      ...
      if (kind === "clips") api.clips.getByCategory = read;
      else api.videos.getByCategory = read;

      renderWithProviders(<CategoryDetailPage />);
      const retry = await screen.findByRole("button", { name: "Retry Twitch" });
      expect(screen.queryByText(/No (Clips|Videos) found for this category/i)).toBeNull();
      read.mockResolvedValue({ success: true, availability: "available", data: [] });
      await userEvent.click(retry);
      expect(await screen.findByText(/No (Clips|Videos) found for this category/i)).toBeVisible();
      expect(screen.queryByRole("button", { name: "Retry Twitch" })).toBeNull();
    }
  );

  it("offers explicit continuation for a filtered empty page without an automatic fetch loop", async () => {
    searchState.tab = "videos";
    searchState.platform = "twitch";
    const api = installElectronAPIMock();
    api.videos.getByCategory = vi
      .fn()
      ...
      .mockResolvedValueOnce({ success: true, availability: "available", data: [] });
    renderWithProviders(<CategoryDetailPage />);
    const more = await screen.findByRole("button", { name: "Load more Videos" });
    expect(api.videos.getByCategory).toHaveBeenCalledTimes(1);
    expect(screen.queryByText(/No Videos found/i)).toBeNull();
    await userEvent.click(more);
    expect(await screen.findByText(/No Videos found/i)).toBeVisible();
    expect(api.videos.getByCategory).toHaveBeenLastCalledWith(
      expect.objectContaining({ cursor: "recorded-later" })
    );
    expect(api.videos.getByCategory).toHaveBeenCalledTimes(2);
  });
  ...
  it("keeps Videos active and renders item-routed Video cards instead of the Live grid", async () => {
    ...
    expect(
      screen.getByText("Kick videos are from channels currently live in this category.")
    ).toBeVisible();
  });
