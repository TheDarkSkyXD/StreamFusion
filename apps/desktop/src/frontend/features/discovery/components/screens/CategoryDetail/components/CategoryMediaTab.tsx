...
import {
  ...
  useCategoryMedia,
} from "@/features/discovery/components/hooks/queries/useCategoryMedia";
import { useFollowedClipPlayback } from "@/features/discovery/components/hooks/queries/useFollowedContent";
...
export function CategoryMediaTab({
  ...
  useEffect(() => {
    ...
    if (!target || !hasNextPage || items.length === 0) return;
  ...
  return (
    <section
      ...
      className="space-y-4"
    >
      {kind === "videos" && platformScope !== "twitch" && kickCategoryId.trim() && (
        <p className="text-sm text-[var(--color-foreground-muted)]">
          {t("discovery.category.kickVideosCoverage")}
        </p>
      )}
      ...
      {items.length === 0 && failures.length === 0 && !hasNextPage ? (
      ...
      {hasNextPage && items.length === 0 && failures.length === 0 && (
        <button
          type="button"
          ...
          className="min-h-10 rounded-md bg-[var(--color-background-elevated)] px-4 font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
        >
          {t("discovery.category.loadNextMedia", { kind: t(`discovery.${kind}`) })}
        </button>
      )}
      {isFetchingNextPage && (
        <div
