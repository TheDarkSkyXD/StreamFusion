...
import { savePersistedChannelMetadata } from "../../../data/queries/persisted-channel-lru";
...
export function useFollowedChannels(platform: Platform, options: { enabled?: boolean } = {}) {
  ...
  const query = useQuery({
    queryKey,
    queryFn: async () => {
      const response = await discoveryReader.channels.getFollowed({ platform });
      if (response.error) {
...
export function channelByUsernameQueryOptions(username: string, platform: Platform) {
  return queryOptions({
    ...
    queryFn: async () => {
      const response = await discoveryReader.channels.getByUsername({ username, platform });
      if (response.error) {
        throw new Error(response.error);
      }
