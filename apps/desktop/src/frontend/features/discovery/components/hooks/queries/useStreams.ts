...
import { deletePersistedSnapshot, savePersistedSnapshot } from "../../../data/queries/persisted-snapshot";
...
export function useTopStreams(platform?: Platform, limit: number = 20) {
  ...
  const query = useQuery({
    queryKey,
    queryFn: async () => {
      const response = await discoveryReader.streams.getTop({ platform, limit });
...
export function useFollowedStreams(
  ...
  const query = useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      const response = await discoveryReader.streams.getFollowed({ platform });
...
export function getStreamByChannelQueryOptions(username: string, platform: Platform) {
  ...
  return queryOptions({
    queryKey,
    queryFn: async () => {
      const response = await discoveryReader.streams.getByChannel({ username, platform });
      if (!response.success) {
