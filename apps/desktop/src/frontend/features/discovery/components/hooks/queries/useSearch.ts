...
import type { SearchResultCollection } from "../../../domain/search/search-result-validation";
import { logger } from "../../../../../renderer/logging/logger";
import { i18n } from "@/i18n";
import {
  type ProgressiveSearchItem,
  type ProgressiveSearchKind,
  type ProgressiveSearchRequest,
  type ProgressiveSearchResponse,
} from "@/features/discovery/capabilities/search-session";
import { discoveryReader } from "@/features/discovery/composition/discovery-reader";
import { discoverySearchSession } from "@/features/discovery/composition/discovery-search-session";

import { useQueryCachePerformance } from "../../../data/queries/cache-performance";
import { getQueryCacheOptions } from "../../../data/queries/cache-policy";
import {
  savePersistedSearchPage,
  type PersistedSearchItem,
  type PersistedSearchKind,
  usePersistedSearchPage,
} from "../../../data/queries/persisted-search-lru";
import {
  savePersistedSearchResult,
  usePersistedSearchResult,
} from "../../../data/queries/persisted-search-results-lru";
...
function getProgressiveSearchMethod<T extends ProgressiveSearchItem>(
  ...
  const methods = {
    streams: discoverySearchSession.streams,
    videos: discoverySearchSession.videos,
    clips: discoverySearchSession.clips,
  };
  return methods[kind] as unknown as (
...
function useProgressiveSearch<T extends ProgressiveSearchItem>({
  ...
  useEffect(() => {
    ...
    return () => {
      void discoverySearchSession.cancel({ requestId: sessionId }).catch(() => undefined);
    };
  }, [active, sessionId]);
...
export function useSearchChannels(
  ...
  const result = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam, signal }) => {
      const response = await discoverySearchSession.channels({
        query: searchQuery,
        platform,
        limit,
        after: pageParam,
...
export function useSearchCategories(
  ...
  const result = useInfiniteQuery({
    ...
    initialPageParam: undefined as string | undefined,
    queryFn: async ({ pageParam, signal }) => {
      const response = await discoveryReader.categories.search({
        query: searchQuery,
        platform,
        limit,
        after: pageParam,
  ...
  const result = useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      const cancel = () => {
        ...
        void discoverySearchSession.cancel({ requestId });
      };
      signal.addEventListener("abort", cancel, { once: true });

      try {
        const response = await discoverySearchSession.all({
          query: normalizedQuery,
          platform,
          limit,
