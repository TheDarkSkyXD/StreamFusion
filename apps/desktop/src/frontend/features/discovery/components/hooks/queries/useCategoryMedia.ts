import { discoveryMediaReader } from "@/features/discovery/composition/discovery-media-reader";
...
function usePlatformCategoryMedia(
  ...
  const query = useInfiniteQuery({
    ...
    queryFn: async ({
      pageParam,
      signal,
    }): Promise<{ items: CategoryMediaItem[]; cursor?: string }> => {
      ...
      const response =
        kind === "clips"
          ? await discoveryMediaReader.clips.getByCategory({
          ...
          : await discoveryMediaReader.videos.getByCategory({
              platform: source.platform,
              categoryId: source.categoryId,
              categorySlug: source.categorySlug,
              categoryName: source.categoryName,
      ...
      signal.throwIfAborted();
      if (response.cursor && response.cursor === pageParam) {
        throw new Error("Category media returned a repeated page cursor.");
      }
      if (
        !response.success ||
    ...
    initialPageParam: "",
    getNextPageParam: (lastPage, _pages, _pageParam, pageParams) =>
      lastPage.cursor && !pageParams.includes(lastPage.cursor) ? lastPage.cursor : undefined,
    enabled,
    retry: false,
    networkMode: "always",
  ...
  const paginationError = useMemo(() => {
    const cursor = query.data?.pages.at(-1)?.cursor;
    return cursor && query.data?.pageParams.includes(cursor)
      ? new Error("Category media returned a repeated page cursor.")
      : null;
  }, [query.data]);
  return { ...query, error: query.error ?? paginationError };
}
...
export function useCategoryMedia({
  ...
  return {
    ...
    hasNextPage: selectedQueries.some(({ query }) => !query.error && query.hasNextPage),
    isFetchingNextPage: selectedQueries.some(({ query }) => query.isFetchingNextPage),
    fetchNextPage: async () => {
      await Promise.all(
        selectedQueries
          .filter(({ query }) => !query.error && query.hasNextPage && !query.isFetchingNextPage)
