import { snapshotStorage } from "@/features/discovery/composition/snapshot-storage";
...
import { isValidUnifiedStream } from "@/features/discovery/domain/search/search-result-validation";
...
import { hydratePersistedChatHistory } from "@/features/chat/components/state/persisted-chat-history";
import { getPersistedChannelEntries, hydratePersistedChannelLru } from "../../../data/queries/persisted-channel-lru";
import { sanitizePersistedCategoryCatalog } from "../../../data/queries/persisted-category-catalog";
import { getPersistedSearchEntries, hydratePersistedSearchLru } from "../../../data/queries/persisted-search-lru";
import {
...
import { loadPersistedSnapshot } from "../../../data/queries/persisted-snapshot";
...
async function readSlot<T>(
  ...
  const value = await snapshotStorage.get(`${PREFIX}${slot}`);
  if (
    typeof value !== "object" ||
    value === null ||
    !("version" in value) ||
