...
import { getFollowRepository } from "../../composition/follow-repository";
...
import { channelsMatch, Platform } from "@streamfusion/core/platform";
import type {
...
let subscribedFollowsApi: FollowRepository | undefined;
...
function ensureAccountWriteSubscription(): void {
  const followsApi = getFollowRepository();
...
interface FollowState {
   ...
   * `authenticationRepository.getActiveFollowsByPlatform`.
   */
...
export const useFollowStore = create<FollowState>()((set, get) => ({
  ...
  followChannel: async (channel) => {
    ...
    try {
      ...
      try {
        const added = await getFollowRepository().add({
  ...
  followAccountChannel: async (channel) => {
    ...
    try {
      const result = await getFollowRepository().writeAccount(
  ...
  unfollowChannel: async (channel) => {
    ...
    try {
      ...
      const source =
        ...
        currentSources.get(`${followToRemove.platform}:${followToRemove.username.toLowerCase()}`);
      ...
      try {
        ...
        const backendFollows = await getFollowRepository().getAll();
        ...
        for (const m of matches) {
          await getFollowRepository().remove(m.id);
        }
  ...
  getFollowSource: (channel) => {
    const state = get();
    const storedFollow = state.localFollows.find((candidate) => channelsMatch(candidate, channel));
  ...
  hydrate: async (_options = {}) => {
    try {
      ...
      const getAccountWrites = getFollowRepository().getAccountWrites;
      const [follows, accountWritesResult] = await Promise.all([
        getFollowRepository().getAll(),
      ...
      for (const write of accountWrites) {
        ...
        const channel = channels.find((candidate) => channelsMatch(candidate, target)) ?? target;
        if (
          !pendingAccountActions.some(
            (pending) => pending.action === write.action && channelsMatch(pending.channel, channel)
          )
        ) {
