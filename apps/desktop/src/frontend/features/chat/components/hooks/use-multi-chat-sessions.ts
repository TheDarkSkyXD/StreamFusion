...
} from "@/features/chat/composition/emote-runtime";
...
import { channelByUsernameQueryOptions } from "../../../discovery/components/hooks/queries/useChannels";
import { logger } from "@/renderer/logging/logger";
import type { MultiChatChannel } from "../state/multi-chat-feed";
import { registerChatMessageRoute } from "./use-chat-message-router";
...
function loadOptionalChatDecorations({ chat, channel }: ResolvedMultiChatChannel): void {
  void (async () => {
    ...
    const token = await getChatSessionAccess().auth.getToken("kick");
