...
vi.mock("@/features/moderation/components/hooks/useModLog", () => ({
  useModLog: () => ({
...
describe("UserPopout", () => {
  ...
  // Guards against unrelated OAuth grants hiding history or bypassing action verification.
  it("keeps history visible with minimal grants while a denied timeout stays unavailable", async () => {
    mockedUseUserProfile.mockReturnValue(pendingProfileState());
    useAuthStore.setState({
      twitchUser: {
        id: "moderator-1",
    ...
    const createTimeoutSnapshot = vi.fn().mockResolvedValue({
      state: "unavailable",
      reason: "unauthorized",
    });
    const submitTimeout = vi.fn();
    window.electronAPI.moderation = { createTimeoutSnapshot, submitTimeout };

    renderPopout();
    expect(await screen.findByRole("heading", { name: "Moderation history" })).toBeVisible();
    await waitFor(() => expect(createTimeoutSnapshot).toHaveBeenCalled());
    expect(screen.queryByRole("button", { name: "Reconnect Twitch" })).toBeNull();
    expect(screen.queryByRole("button", { name: "Timeout user" })).toBeNull();
    expect(useReconnectDialogStore.getState().isOpen).toBe(false);
    expect(submitTimeout).not.toHaveBeenCalled();
  });
