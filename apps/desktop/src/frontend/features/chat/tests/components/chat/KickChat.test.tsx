...
vi.mock("@/features/chat/components/state/persisted-chat-history", () => ({
...
vi.mock("@/features/moderation/adapters/electron/mod-log-writer", () => ({
  modLogWriter: {
...
vi.mock("@/features/moderation/components/hooks/useIsKickMod", () => ({
...
vi.mock("@/features/chat/adapters/browser/kick-chat", () => ({
  kickChatService: {
...
vi.mock("@/features/chat/adapters/browser/kick-predictions-service", () => ({
...
vi.mock("@/features/chat/composition/emote-runtime", () => ({
...
vi.mock("@/features/chat/components/state/chat-store", () => {
...
vi.mock("@/features/chat/components/state/emote-store", () => {
  const state = {
...
import { kickChatService } from "@/features/chat/adapters/browser/kick-chat";
...
describe("KickChat", () => {
  beforeEach(() => {
    ...
    api.kickChat.moderateTimeout = timeoutKickUserMock;
    api.kickChat.moderateUnban = unbanKickUserMock;
    api.kickChat.setMode = setKickChatModeMock;
    ...
    storeState.connectionStatus.kick.channels = [];
  ...
  it("shows connection progress before a pending transport resolves and retries through lifecycle cleanup", async () => {
    vi.mocked(kickChatService.connect).mockImplementationOnce(() => new Promise(() => {}));
    const { rerender } = renderKickChat(<KickChat channel="xqc" chatroomId={12345} />);
    expect(screen.getByTestId("kick-chat-connection-status")).toHaveTextContent(
      "Connecting to Kick chat"
    );
    await waitFor(() => expect(kickChatService.connect).toHaveBeenCalledTimes(1));
    expect(kickChatService.joinChannel).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole("button", { name: "Retry chat" }));
    await waitFor(() => expect(kickChatService.connect).toHaveBeenCalledTimes(2));
    expect(kickChatService.release).toHaveBeenCalledWith("xqc");
    await waitFor(() =>
      expect(kickChatService.joinChannel).toHaveBeenCalledWith("xqc", 12345, undefined)
    );
    storeState.connectionStatus.kick.state = "connected";
    storeState.connectionStatus.kick.channels = ["another-room"];
    rerender(<KickChat channel="xqc" chatroomId={12345} />);
    expect(screen.getByTestId("kick-chat-connection-status")).toHaveTextContent(
      "Connecting to Kick chat"
    );
    storeState.connectionStatus.kick.channels = ["xqc"];
    rerender(<KickChat channel="xqc" chatroomId={12345} />);
    await waitFor(() =>
      expect(screen.queryByTestId("kick-chat-connection-status")).not.toBeInTheDocument()
    );
    storeState.connectionStatus.kick.state = "reconnecting";
    rerender(<KickChat channel="xqc" chatroomId={12345} />);
    expect(screen.getByTestId("kick-chat-connection-status")).toHaveTextContent(
      "Kick chat is unavailable. Reconnecting"
    );
  });

  it("shows a recoverable rail error when transport connection fails", async () => {
    vi.mocked(kickChatService.connect).mockRejectedValueOnce(new Error("Transport unavailable"));
    renderKickChat(<KickChat channel="xqc" chatroomId={12345} />);
    await waitFor(() =>
      expect(screen.getByTestId("kick-chat-connection-status")).toHaveTextContent(
        "Could not connect to Kick chat."
      )
    );
