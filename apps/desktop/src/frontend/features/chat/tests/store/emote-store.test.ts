...
describe("emote-store persisted picker state", () => {
  ...
  it("keeps Twitch and Kick accounts isolated across an app restart", async () => {
    ...
    vi.resetModules();
    const { useEmoteStore: freshEmoteStore } = await import("@/features/chat/components/state/emote-store");
  ...
  it("migrates a legacy storage blob for one-time Twitch and Kick account claims", async () => {
    ...
    vi.resetModules();
    const { useEmoteStore: migratedStore } = await import("@/features/chat/components/state/emote-store");
