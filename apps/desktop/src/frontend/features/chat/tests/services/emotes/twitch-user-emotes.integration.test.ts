...
import { registerTwitchApiRoutes } from "@backend/features/chat/routes/twitch-api-routes";
import { EmoteManager } from "@backend/../frontend/features/chat/adapters/browser/emote-manager";
import { twitchEmoteProvider } from "@backend/../frontend/features/chat/adapters/browser/emotes/twitch-emotes";
import type { Emote, EmoteProviderService } from "@backend/../frontend/features/chat/capabilities/emote-types";
...
describe("Twitch user-emote capability through the global loader", () => {
  it("loads restored-session user emotes across an opaque long pagination cursor", async () => {
    ...
    registerTwitchApiRoutes({ service });
