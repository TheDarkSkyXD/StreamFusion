...
import { twitchChatService } from "@backend/../frontend/features/chat/adapters/browser/twitch-chat";
import { __resetTwitchPinPollers, toNormalized } from "@backend/../frontend/features/chat/adapters/browser/twitch-pin-poller";
...
describe("transport", () => {
  it("uses the Electron chat bridge when available so renderer fetch does not log net::ERR failures", async () => {
    ...
    const { startTwitchPinPolling } = await import("@backend/../frontend/features/chat/adapters/browser/twitch-pin-poller");
