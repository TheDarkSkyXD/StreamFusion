import type { Meta, StoryObj } from "@storybook/react-vite";
import { fn } from "storybook/test";

import { STORY_CHANNEL_ID, seedChatStoryStores } from "../chat-story-fixtures";
import { ThirdPartyEmoteButton } from "../../../../components/chat/input/ThirdPartyEmoteButton";

seedChatStoryStores();

const meta = {
  title: "Components/Chat/Input/Third-Party Emote Button",
  component: ThirdPartyEmoteButton,
  parameters: { layout: "centered" },
  decorators: [
    (Story) => (
      <div className="flex h-12 items-stretch overflow-hidden rounded-md bg-[#191919]">
        <Story />
      </div>
    ),
  ],
  args: {
    platform: "twitch",
    channel: "novaarcade",
    channelId: STORY_CHANNEL_ID,
    isOpen: false,
    onOpenRequest: fn(),
    onEmoteSelect: fn(),
  },
} satisfies Meta<typeof ThirdPartyEmoteButton>;

export default meta;
type Story = StoryObj<typeof meta>;

export const Twitch: Story = {};

export const Kick: Story = {
  args: {
    platform: "kick",
    channel: "pixelnomad",
    kickUserId: "7419",
  },
};

export const OpenPicker: Story = {
  args: { isOpen: true },
};

export const Disabled: Story = {
  args: { disabled: true },
};
