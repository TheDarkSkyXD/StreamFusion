import { getDiagnosticsClient } from "@/features/settings/composition/settings-services";
import { useCallback, useEffect, useRef, useState } from "react";

import { startRendererDiagnosticsReporter } from "../../composition/diagnostics/renderer-diagnostics-reporter";
...
export function useDiagnosticsWorkspace(view: DiagnosticsView) {
  ...
  useEffect(() => {
    ...
    const removeSnapshotListener = getDiagnosticsClient().onSnapshotChanged((event) => {
      if (event.leaseId !== leaseIdRef.current) return;
      setState({ kind: "ready", snapshot: event.snapshot });
    });

    void getDiagnosticsClient()
      ...
      .then(async (reply) => {
        ...
        if (cancelled) {
          await getDiagnosticsClient().closeLease(reply.value.leaseId);
          return;
        }
    ...
    return () => {
      ...
      setLeaseId(null);
      if (leaseId) void getDiagnosticsClient().closeLease(leaseId);
    };
  }, []);
  ...
  const refresh = useCallback(async () => {
    const leaseId = leaseIdRef.current;
    if (!leaseId) return;
    const reply = await getDiagnosticsClient().refresh(leaseId);
    if (reply.kind === "ok") {
      setState({ kind: "ready", snapshot: reply.value });
    } else {
      setState((current) => ({
