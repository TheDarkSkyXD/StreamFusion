...
import type {
  ...
  UpdateProgress,
  UpdateStatus,
} from "@/features/settings/components/state/update-store";
import { useUpdateStore } from "@/features/settings/components/state/update-store";
import { getUpdateController } from "@/features/settings/composition/update-controller";
...
export function useUpdater(): UseUpdaterReturn {
  ...
  useEffect(() => {
    const updater = getUpdateController();
    if (!updater) {
      return;
    }
  ...
  const setAllowPrerelease = useCallback(
    async (allow: boolean) => {
      ...
      try {
        const result = await updater.setAllowPrerelease(allow);
  ...
  const setAutoCheckEnabled = useCallback(
    async (enabled: boolean) => {
      ...
      try {
        const result = await updater.setAutoCheck({ enabled });
  ...
  const setCheckFrequency = useCallback(
    async (frequency: CheckFrequency) => {
      ...
      try {
        const result = await updater.setAutoCheck({ frequency });
        storeSetAutoCheckEnabled(result.autoCheckEnabled);
        storeSetCheckFrequency(result.checkFrequency);
      } catch (err) {
  ...
  const setUpdateCheckUrl = useCallback(
    async (url: string) => {
      ...
      if (!updater) return;
      try {
        const result = await updater.setAutoCheck({ updateCheckUrl: url });
