import { getPlatformHealthFeed } from "@/features/settings/composition/settings-services";
/**
...
import { invalidatePlatformRecoveryCaches } from "@/features/discovery/data/queries/cache-invalidation";
import { queryClient } from "@/providers/query-provider";

import type { PlatformHealthSnapshot } from "../../capabilities/platform-health-feed";
export type PlatformHealth = PlatformHealthSnapshot["kick"];
export type StatusPageDetail = NonNullable<PlatformHealthSnapshot["details"]>["kick"];
...
export function usePlatformHealth(): PlatformHealthState {
  ...
  useEffect(() => {
    const bridge = getPlatformHealthFeed();
    if (!bridge) return;
