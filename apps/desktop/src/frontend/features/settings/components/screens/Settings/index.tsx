...
import {
  ...
  getPlaybackBudgetSettings,
  getNotificationCoverageReader,
  getProxySettings,
...
} from "@/features/playback/adapters/browser/twitch/twitch-adblock-device-id";
...
import { useAppVersion, useAppVersionInfo } from "@/features/settings/components/hooks/useElectron";
import { useUpdater } from "@/features/settings/components/hooks/useUpdater";
import { useAfterFirstPaint } from "@/hooks/useAfterFirstPaint";
import { useAuthError } from "@/features/auth/components/hooks/useAuth";
import {
...
import {
  ...
  useAppStore,
} from "@/features/shell/components/state/app-store";
...
export function SettingsPage() {
  ...
  const handlePlaybackBudgetChange = (next: number) => {
    ...
    void getPlaybackBudgetSettings()?.setPlaybackBudget?.(next);
    notifySettingsSaved();
  };
  ...
  const handleProxySave = async () => {
    ...
    if (proxyPassword !== "") {
      const result = await getProxySettings()!.setCredentials({
    ...
    const applyResult = await getProxySettings()!.apply(config);
  ...
  const handleProxyClearCredentials = async () => {
    const result = await getProxySettings()!.setCredentials(null);
    setProxyHasCredentials(result.hasCredentials);
    setProxyUsername("");
    setProxyPassword("");
  };
...
function ApiTokenPanel({
  ...
  const validate = useCallback(async () => {
    setLoading(true);
    try {
      const result = await getSessionInspector()!.tokenStatus(platform);
      setStatus(result);
    } catch {
