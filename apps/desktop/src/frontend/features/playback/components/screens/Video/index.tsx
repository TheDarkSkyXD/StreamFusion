...
import type { PlaybackMediaReader } from "@/features/playback/capabilities/playback-media-reader";
...
import { useVodLiveLink } from "@/features/playback/components/hooks/useVodLiveLink";
...
import { createChatReplayPlaybackStore } from "@/features/chat/components/state/chat-replay-playback-store";
...
export function VideoPage() {
  ...
  const playbackRequestsRef = useRef(
    new Map<string, ReturnType<PlaybackMediaReader["videos"]["getPlaybackUrl"]>>()
  );
  const metadataRequestsRef = useRef(
    new Map<string, ReturnType<PlaybackMediaReader["videos"]["getMetadata"]>>()
  );
  ...
  useEffect(() => {
    ...
    const getPlaybackRequest = () => {
      ...
      const request = getPlaybackMediaReader().videos.getPlaybackUrl({
    ...
    const getMetadataRequest = () => {
      ...
      const request = getPlaybackMediaReader().videos.getMetadata({
        platform: platform as "twitch" | "kick",
        videoId,
      });
  ...
  useEffect(() => {
    ...
    const fetchRelated = async () => {
      ...
      try {
        const result = await getPlaybackMediaReader().videos.getByChannel({
          platform: routePlatform,
          channelName,
          limit: 100,
        });
