...
import type { StreamPlayback } from "@/features/playback/capabilities/media-types";
import { desktopPlaybackSource } from "../../adapters/electron/playback-source";
...
async function fetchPlaybackUrlFromBackend(
  ...
  return desktopPlaybackSource.resolve({ platform, identifier, intent });
}
...
export function prefetchStreamPlayback(
  ...
  if (!identifier || !desktopPlaybackSource.isAvailable()) return undefined;
