...
describe("useShareAction", () => {
  ...
  it.each([
    "https://www.twitch.tv/bobross/clip/SpiritedGreyMountains-123_abc",
    "https://twitch.tv/bobross/clip/SpiritedGreyMountains-123_abc/",
  ])("accepts the provider's canonical Twitch channel clip link: %s", async (shareUrl) => {
    writeText.mockResolvedValue(undefined);
    const { result } = renderHook(() =>
      useShareAction({ shareUrl, isPlaybackReady: true, contentLabel: "Clip" })
    );
    expect(result.current.canShare).toBe(true);
    await act(async () => result.current.share());
    expect(writeText).toHaveBeenCalledWith(shareUrl);
  ...
  it.each([
    "https://www.twitch.tv/bobross/clip/",
    "https://www.twitch.tv/bobross/clip/Slug/extra",
    "https://www.twitch.tv/bobross/clips/Slug",
    "https://www.twitch.tv.evil.example/bobross/clip/Slug",
    "https://www.twitch.tv/bobross/clip/Slug%2Fextra",
  ])("rejects noncanonical channel clip lookalikes: %s", (shareUrl) => {
    const { result } = renderHook(() =>
      useShareAction({ shareUrl, isPlaybackReady: true, contentLabel: "Clip" })
    );
    expect(result.current.canShare).toBe(false);
