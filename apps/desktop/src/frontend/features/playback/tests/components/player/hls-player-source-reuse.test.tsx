...
const fakeHlsModule = vi.hoisted(() => {
  ...
  class FakeHls {
    ...
    destroy = vi.fn(() => this.listeners.clear());
    startLoad = vi.fn();
    stopLoad = vi.fn();
...
describe("HlsPlayer source reuse (slice 09)", () => {
  ...
  it("retains one active quality listener across repeated source switches", () => {
    const onActiveQualityChange = vi.fn();
    const { rerender, unmount } = render(
      <HlsPlayer src="https://x.test/0.m3u8" isLive onActiveQualityChange={onActiveQualityChange} />
    );
    const hls = fakeHlsModule.instances[0];

    for (let index = 1; index <= 100; index += 1) {
      rerender(
        <HlsPlayer
          src={`https://x.test/${index}.m3u8`}
          isLive
    ...
    expect(fakeHlsModule.instances).toHaveLength(1);
    const event = fakeHlsModule.FakeHls.Events.LEVEL_SWITCHED;
    expect(hls.listeners.get(event)?.size).toBe(1);
    act(() => {
      for (const listener of hls.listeners.get(event) ?? []) listener(event, { level: 2 });
    });
    expect(onActiveQualityChange).toHaveBeenCalledExactlyOnceWith("2");

    unmount();
    expect(hls.listeners.size).toBe(0);
  });
