...
} from "@/features/moderation/components/state/moderated-channels-store";
...
const TWITCH_ROLE_SCOPES: readonly string[] = [];
...
function canonicalScopes(platform: Platform): readonly string[] {
  return platform === "twitch" ? TWITCH_ROLE_SCOPES : KICK_APP_SCOPES;
}
...
export function useModerationAuthority(
  ...
  useEffect(() => {
    ...
    if (forceModScopes) {
      setScopeCheck({
        ...
        state: "confirmed",
        missingScopes: [],
        grantedScopes: platform === "twitch" ? TWITCH_APP_SCOPES : KICK_APP_SCOPES,
      });
      return;
    }
  ...
  const retryAuthority = useCallback(async () => {
    if (platform === "twitch") {
      ...
      const result = await getModerationServices().kickChat.getViewerRole(channelSlug);
  ...
  return {
    ...
    requestScopes: (scopes) =>
      useReconnectDialogStore
        .getState()
        .open({ platform, missingScopes: scopes, onReconnected: retry }),
  };
}
