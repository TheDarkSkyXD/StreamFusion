...
import { useResolveTwitchChannel } from "@/features/moderation/components/hooks/useResolveTwitchChannel";
...
export function ModChannelPage({ platform, channel }: ModChannelPageProps) {
  ...
  const workspaceIdentity = `${platform}:${platform === "twitch" ? twitchUser?.id : kickUser?.id}:${channelId}`;
  const [openedWorkspace, setOpenedWorkspace] = useState<string | null>(null);
  useEffect(() => {
    if (hasModerationAuthority) setOpenedWorkspace(workspaceIdentity);
    else if (moderationAuthority.state === "hidden") setOpenedWorkspace(null);
  }, [hasModerationAuthority, moderationAuthority.state, workspaceIdentity]);
  const refreshingOpenWorkspace =
    openedWorkspace === workspaceIdentity &&
    (moderationAuthority.state === "checking" || moderationAuthority.state === "unverifiable");
  const canRenderWorkspace =
    Boolean(channelId && retentionScope) &&
    (isOwnBroadcaster || hasModerationAuthority || refreshingOpenWorkspace);
      ...
      {isTwitchResolving || isKickResolving ? (
        ...
        <>
          ...
          <ChannelWorkspace
            ...
            grantedScopes={
              moderationAuthority.state === "authorized" ? moderationAuthority.grantedScopes : []
            }
            requestScopes={
              moderationAuthority.state === "authorized"
                ? moderationAuthority.requestScopes
                : undefined
            }
