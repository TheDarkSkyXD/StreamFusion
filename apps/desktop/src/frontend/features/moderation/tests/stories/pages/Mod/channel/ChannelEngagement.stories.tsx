...
import { ChannelEngagement } from "../../../../../components/screens/Mod/channel/ChannelEngagement";
...
export const Loading: Story = {
  ...
  play: async ({ canvasElement }) => {
    await expect((await within(canvasElement).findAllByText("Loading…"))[0]).toBeInTheDocument();
  },
};
...
export const LockedPrediction: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(await canvas.findByText("LOCKED")).toBeInTheDocument();
    await expect(canvas.getByText("Can the final boss be defeated?")).toBeInTheDocument();
  },
};

export const UnavailableShowsError: Story = {
  render: () => <ChannelEngagementStoryCanvas state="unavailable" />,
  play: async ({ canvasElement }) => {
    await expect((await within(canvasElement).findAllByRole("alert"))[0]).toBeInTheDocument();
  },
};
