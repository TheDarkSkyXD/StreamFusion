...
import {
  ModWorkspace,
  type ModWidget,
} from "@/features/moderation/components/screens/Mod/channel/workspace/ModWorkspace";

const workspaceStyles = readFileSync(
  "src/frontend/features/moderation/components/screens/Mod/channel/workspace/mod-workspace.css",
  "utf8"
);
...
describe("ModWorkspace", () => {
  ...
  it.each([
    ...
    async (platform, anchorTitle) => {
      ...
      expect(screen.getByRole("dialog", { name: "Mod Actions" })).toBeInTheDocument();
      expect(localStorage.getItem(storageKey)).not.toContain("mod-log");
  ...
  it("previews and closes dock tools without changing layout, mounted media or persistence", async () => {
    const available = [
      ...widgets("twitch"),
      {
        ...
        title: "Activity",
        icon: <span>A</span>,
        content: <StablePanel id="activity" />,
      },
      {
        id: "community" as const,
        title: "Community",
        icon: <span>C</span>,
        content: <StablePanel id="community" />,
      },
    ];
    renderWithProviders(
      <ModWorkspace platform="twitch" storageKey={storageKey} widgets={available} />
    );
    await screen.findAllByRole("separator");
    const video = getWidget("Video");
    const chat = getWidget("Chat");
    ...
    expect(screen.getByRole("dialog", { name: "Activity" })).toHaveFocus();
    fireEvent.click(screen.getByRole("button", { name: "Community" }));
    expect(screen.queryByRole("dialog", { name: "Activity" })).toBeNull();
    expect(screen.getByRole("dialog", { name: "Community" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Close Community preview" }));
    expect(screen.queryByRole("dialog")).toBeNull();
    expect(screen.getByRole("button", { name: "Community" })).toHaveFocus();
    fireEvent.click(screen.getByRole("button", { name: "Activity" }));
    fireEvent.keyDown(screen.getByRole("dialog"), { key: "Escape" });
    expect(screen.queryByRole("dialog")).toBeNull();
    expect(getWidget("Video")).toBe(video);
    expect(getWidget("Chat")).toBe(chat);
    expect(video.getAttribute("style")).toBe(before);
    expect(localStorage.getItem(storageKey)).toBe(saved);
    expect(mounts.mock.calls.filter(([id]) => id === "video")).toHaveLength(1);
  });

  it("adds previews without remounting content and stops when another pane cannot fit", async () => {
    const ids = [
      "activity",
      "community",
      "whispers",
    ...
    for (const id of ids) {
      ...
      const content = screen.getByTestId(`${id}-content`);
      const add = screen.getByRole("button", { name: "Add to workspace" });
      if (add.hasAttribute("disabled")) {
        denied = true;
        expect(screen.getByText(/Close another panel or enlarge/)).toBeInTheDocument();
        fireEvent.click(add);
        expect(localStorage.getItem(storageKey)).toBe(saved);
      ...
      fireEvent.click(add);
      expect(screen.queryByRole("dialog")).toBeNull();
      expect(screen.getByTestId(`${id}-content`)).toBe(content);
      for (const element of document.querySelectorAll<HTMLElement>(".mod-workspace-widget")) {
        expect(numericStyle(element, "width")).toBeGreaterThanOrEqual(240);
        expect(numericStyle(element, "height")).toBeGreaterThanOrEqual(180);
      }
    }
