...
import { installNativeStderrIntercept } from "./logging/native-stderr-intercept";
import { installNetworkDevtoolsRecorder } from "./logging/network-devtools-recorder";
import { installNetworkLogRouter } from "./logging/network-log-router";
import {
...
} from "./protocols/twitch-clip-media-protocol";
import { TWITCH_CLIP_MEDIA_SCHEME } from "./protocols/twitch-clip-media-url";
import {
...
function initializeBeforeReady(): void {
  ...
  // real backend logger. They import `@shared/utils/cross-logger` instead of
...
async function initializeReady(): Promise<void> {
  if (process.env.STREAMFUSION_BROWSER_DEV === "1") {
    const { startConfiguredDevRelay } = await import("./dev-relay/dev-relay-runtime");
...
app.on("child-process-gone", (_event, details) => {
  ...
  void import("./api/unified/platform-health").then((m) => {
