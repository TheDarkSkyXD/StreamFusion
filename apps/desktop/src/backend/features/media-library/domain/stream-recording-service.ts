...
import type {
  ...
  StreamRecordingLifecycle,
  StreamRecordingSectionFinalizer,
} from "../capabilities/stream-recording-lifecycle";
import type { StreamRecordingSessionStore } from "../capabilities/stream-recording-store";
...
export function createStreamRecordingService({
  ...
  resolveFfmpegPath,
  startRecorder,
  sectionFinalizer = lifecycle.sectionFinalizer,
  probeArtifact = lifecycle.probeArtifact,
  cleanupSections = lifecycle.cleanupSections,
  discardArtifacts = lifecycle.discardArtifacts,
  cleanupFailedArtifact = lifecycle.cleanupFailedArtifact,
  cleanupAbortedSection = lifecycle.cleanupAbortedSection,
  isRecordingSectionAvailable = lifecycle.isRecordingSectionAvailable,
  verifyArtifactIdentity = lifecycle.verifyArtifactIdentity,
  ...
  createSectionPath = lifecycle.createSectionPath,
  now = defaultNow,
  sleep = defaultSleep,
  ...
  lifecycle: StreamRecordingLifecycle;
  ...
  async function rollbackSpawnedRecorder(
    ...
    if (
      ownsSpawnedWriter &&
      lifecycle.isOwnedRecordingSection(
    ...
    try {
      ...
      if (
        ...
        for (const [index, section] of latest.sections.entries()) {
          if (
            !lifecycle.isOwnedRecordingSection(latest.destinationPath, latest.id, index + 1, section) ||
  ...
  return {
    ...
    async discardRecording(sessionId) {
      ...
      const ownedPaths = session.sections.flatMap((section, index) =>
        lifecycle.isOwnedRecordingSection(session.destinationPath, session.id, index + 1, section)
          ? [section.path]
          : []
      );
      ...
      for (const output of outputCandidates) {
        if (
          !lifecycle.isOwnedRecordingOutput(
    ...
    async resumeInterrupted(sessionId) {
      ...
      try {
        for (const [index, section] of session.sections.entries()) {
          const owned = lifecycle.isOwnedRecordingSection(
            session.destinationPath,
            session.id,
            index + 1,
            section
