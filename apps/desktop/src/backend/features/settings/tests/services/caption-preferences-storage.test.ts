...
describe("caption preference storage", () => {
  beforeEach(() => {
    ...
    preferencesRepository.updatePreferences(legacyPreferences as typeof DEFAULT_USER_PREFERENCES);
  });

  it("hydrates and round-trips logical source, model, language, enabled state, and appearance", () => {
    expect(preferencesRepository.getPreferences().captions).toEqual({
      enabled: false,
      source: "platform",
    ...
    preferencesRepository.updatePreferences({ captions });

    expect(preferencesRepository.getPreferences().captions).toEqual(captions);
  });
  ...
  it("uses safe defaults for malformed legacy caption values and preserves unrelated preferences", () => {
    ...
    const hydrated = preferencesRepository.getPreferences();
  ...
  it("stores only logical caption selection fields and preserves unrelated preference fields", () => {
    ...
    const persisted = preferencesRepository.getPreferences();
