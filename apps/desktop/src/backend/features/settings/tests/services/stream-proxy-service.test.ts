...
import {
  ...
  setProxyCredentials,
} from "@backend/features/settings/adapters/electron/stream-proxy-service";
...
describe("credential storage — safeStorage, encrypted, write-only", () => {
  ...
  it("exposes NO function that returns the password to the renderer", async () => {
    ...
    const mod = await import("@backend/features/settings/adapters/electron/stream-proxy-service");
