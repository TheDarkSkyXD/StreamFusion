...
type CertVerifyModule = typeof import("@backend/features/settings/adapters/electron/cert-verify-diagnostics");
...
async function freshModule(): Promise<{ mod: CertVerifyModule; logger: LoggerMock }> {
  vi.resetModules();
  const mod = await import("@backend/features/settings/adapters/electron/cert-verify-diagnostics");
