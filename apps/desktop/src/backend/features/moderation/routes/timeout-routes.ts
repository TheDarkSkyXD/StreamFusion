...
import { createKickTimeoutAuthorityAdapter } from "@backend/features/moderation/adapters/kick/kick-timeout-authority-adapter";
import { createTwitchTimeoutAuthorityAdapter } from "@backend/features/moderation/adapters/twitch/twitch-timeout-authority-adapter";
import { logger } from "@backend/logging/logger";
import { moderationDataRepository } from "@backend/features/moderation/data/moderation-data-repository";
import { createTimeoutModerationService } from "@backend/features/moderation/domain/timeout-moderation-service";
...
async function readWithDevelopmentFixture<T>(
  ...
  if (!app.isPackaged) {
    try {
      ...
      const { getModerationBrowserFixture } = await import("@shared/moderation-fixture-contract");
      const fixture = getModerationBrowserFixture(path, args, search, true);
