...
vi.mock("@backend/features/moderation/data/moderation-data-repository", () => ({
  moderationDataRepository: {
    insert: vi.fn(),
    query: vi.fn(),
    getCoverage: vi.fn(),
    setCoverage: vi.fn(),
...
vi.mock("@backend/features/moderation/adapters/platform/moderation-history-authorization", () => ({
  authorizeModerationHistory: vi.fn(),
}));

import { ipcMain } from "electron";

import { registerModLogHandlers } from "@backend/features/moderation/routes/modlog-routes";
...
describe("MODLOG_INSERT", () => {
  it("persists an authorized provider-originated entry and marks partial coverage", async () => {
    ...
    vi.mocked(moderationDataRepository.insert).mockReturnValue(1);

    const handler = getHandler(IPC_CHANNELS.MODLOG_INSERT);
    const result = await handler(allowedEvent, { entry });

    expect(moderationDataRepository.insert).toHaveBeenCalledWith(entry);
    expect(moderationDataRepository.setCoverage).toHaveBeenCalledWith(
  ...
  it("does not persist an action when current-channel moderator authority is unverified", async () => {
    ...
    expect(moderationDataRepository.insert).not.toHaveBeenCalled();
  });
});
...
describe("MODLOG_QUERY", () => {
  it("fails closed before reading history for a renderer outside the app origin", async () => {
    ...
    expect(moderationDataRepository.query).not.toHaveBeenCalled();
  });
  ...
  it("returns ready records only when persisted coverage is complete", async () => {
    ...
    vi.mocked(moderationDataRepository.query).mockReturnValue(rows);
    vi.mocked(moderationDataRepository.getCoverage).mockReturnValue({
      platform: "twitch",
      channelId: "123",
      coverage: "complete",
    ...
    expect(moderationDataRepository.query).toHaveBeenCalledWith(filters);
  ...
  it("returns verified-empty only for an authorized complete provider query", async () => {
    vi.mocked(moderationDataRepository.query).mockReturnValue([]);
    vi.mocked(moderationDataRepository.getCoverage).mockReturnValue({
      platform: "kick",
      channelId: "123",
      coverage: "complete",
  ...
  it("returns a retryable error instead of verified-empty when storage fails", async () => {
    vi.mocked(moderationDataRepository.query).mockImplementation(() => {
...
describe("MODLOG_SWEEP_RETENTION", () => {
  it("passes now to moderationDataRepository.sweepRetention", () => {
    const now = Date.now();
    vi.mocked(moderationDataRepository.sweepRetention).mockReturnValue(5);

    const handler = getHandler(IPC_CHANNELS.MODLOG_SWEEP_RETENTION);
    const result = handler({}, { now });

    expect(moderationDataRepository.sweepRetention).toHaveBeenCalledWith(now);
    expect(result).toBe(5);
  });

  it("calls sweepRetention with undefined when no now is provided", () => {
    vi.mocked(moderationDataRepository.sweepRetention).mockReturnValue(0);

    const handler = getHandler(IPC_CHANNELS.MODLOG_SWEEP_RETENTION);
    handler({});

    expect(moderationDataRepository.sweepRetention).toHaveBeenCalledWith(undefined);
  });
});

describe("RETENTION_GET", () => {
  it("returns the retention setting for the given scope", () => {
    vi.mocked(moderationDataRepository.getRetention).mockReturnValue(30);

    const handler = getHandler(IPC_CHANNELS.RETENTION_GET);
    const result = handler({}, { scope: "modlog" });

    expect(moderationDataRepository.getRetention).toHaveBeenCalledWith("modlog");
...
describe("RETENTION_SET", () => {
  it("sets the retention setting for the given scope and days", () => {
    ...
    expect(moderationDataRepository.setRetention).toHaveBeenCalledWith("modlog", 90);
  });
  ...
  it("accepts null days to disable retention", () => {
    ...
    expect(moderationDataRepository.setRetention).toHaveBeenCalledWith("modlog", null);
  });
});
