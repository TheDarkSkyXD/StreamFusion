...
import { TokenRefreshError, tokenExchangeService } from "../oauth/token-exchange";
...
function createTwitchOAuth2Session(): OAuth2Session<AuthToken> {
  return createOAuth2Session<AuthToken>({
    credentials: {
      load: async () => authenticationRepository.getToken("twitch"),
      save: async (credential) => {
        authenticationRepository.saveToken("twitch", credential);
      },
      clear: async () => {
...
class TwitchAuthService {
  ...
  scheduleProactiveRefresh(): void {
    const token = authenticationRepository.getToken(this.platform);
   ...
   * Single-pass equivalent of ensureValidToken() + authenticationRepository.getToken()
  ...
  async logout(): Promise<boolean> {
    ...
    authenticationRepository.clearToken(this.platform);
  ...
  async fetchCurrentUser(accessToken?: string): Promise<TwitchUser | null> {
    ...
    const token = accessToken ?? authenticationRepository.getToken(this.platform)?.accessToken;
    ...
    try {
      const tokenIsCurrent = (): boolean =>
        authenticationRepository.getToken(this.platform)?.accessToken === token;
      ...
      authenticationRepository.saveTwitchUser(user);
