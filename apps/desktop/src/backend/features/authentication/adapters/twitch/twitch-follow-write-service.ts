...
} from "@backend/features/authentication/adapters/twitch/follow-endpoints";
import { twitchTransport } from "@backend/api/platforms/twitch/twitch-transport";
import { getUser, getAllFollowedChannels } from "./account-endpoints";
import {
  twitchFollowWriteCredentialService,
  type TwitchFollowWriteCredential,
} from "@backend/features/authentication/adapters/twitch/twitch-follow-write-credential";
...
export function createTwitchFollowWriteService(
  ...
  return new TwitchFollowWriteService({
    storage: authenticationRepository,
    ...
    getCurrentUser: () => getUser(twitchTransport),
    writeTwitchAccountFollow,
    getAllFollowedChannels: () => getAllFollowedChannels(twitchTransport),
    ...dependencies,
  });
}
