...
import { KickLiveNotificationSource } from "../kick/kick-live-notification-source";
import { TwitchLiveEventSubSource } from "../twitch/twitch-live-eventsub-source";
...
export function getLiveNotificationFollows(): LocalFollow[] {
  ...
  for (const platform of ["twitch", "kick"] as Platform[]) {
    for (const follow of [
      ...
      ...authenticationRepository.getGuestFollowsByPlatform(platform),
    ]) {
...
async function fetchTwitchLiveStreams(follows: LocalFollow[]): Promise<UnifiedStream[]> {
  ...
  const { gqlGetStreamsByLogins } =
    await import("@backend/features/discovery/adapters/twitch/twitch-gql-discovery");
...
async function fetchPublicKickStreamBySlug(
  ...
  const { getPublicStreamBySlug } =
    await import("../../../discovery/adapters/kick/stream-endpoints");
  ...
  start(renderer: MainRendererPort): void {
    ...
    if (!this.service) {
      this.service = new LiveNotificationService({
        ...
        getPreferences: () => preferencesRepository.getPreferences(),
        sources: {
    ...
    if (!this.twitchEventSubSource) {
      this.twitchEventSubSource = new TwitchLiveEventSubSource({
        getToken: () => authenticationRepository.getToken("twitch"),
        getUser: () => authenticationRepository.getTwitchUser(),
        getFollows: () => authenticationRepository.getActiveFollowsByPlatform("twitch"),
