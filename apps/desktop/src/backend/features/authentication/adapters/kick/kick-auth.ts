...
import { hasCanonicalKickScopes } from "../../domain/kick-scope-validation";
import { tokenExchangeService, TokenRefreshError } from "../oauth/token-exchange";
...
function createKickOAuth2Session(): OAuth2Session<AuthToken> {
  return createOAuth2Session<AuthToken>({
    credentials: {
      load: async () => authenticationRepository.getToken("kick"),
      save: async (credential) => {
        authenticationRepository.saveToken("kick", credential);
      },
      clear: async () => {
...
class KickAuthService extends EventEmitter {
  ...
  onSystemResume(): void {
    if (!authenticationRepository.getToken(this.platform)) return;
