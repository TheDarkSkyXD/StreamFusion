...
import { createHiddenKickBrowserWindow } from "@backend/api/platforms/kick/kick-hidden-browser-window";
import { transformKickFollowedChannelLegacy } from "@backend/api/platforms/kick/kick-transformers";
import type { KickLegacyApiFollowedChannel } from "@backend/api/platforms/kick/kick-types";
import { KICK_LEGACY_API_V2_BASE } from "@backend/api/platforms/kick/kick-types";
import {
...
import { hasCanonicalKickScopes } from "@backend/features/authentication/domain/kick-scope-validation";
import {
  fetchKickWebApiGet,
...
import { GRID_READY_PREDICATE } from "@backend/features/authentication/utils/kick-follow-grid-predicate";
import { acquireBrowserWindowSlot } from "@backend/features/discovery/adapters/kick/channel-endpoints";

export { GRID_READY_PREDICATE } from "@backend/features/authentication/utils/kick-follow-grid-predicate";
...
async function _doFetch(options: FollowedChannelsOptions): Promise<FollowedChannelsResult> {
  const storedToken = authenticationRepository.getToken("kick");
...
async function tryWebSessionFollowCollection(
  ...
  const viewer = authenticationRepository.getKickUser();
