...
import { liveNotificationService } from "@backend/features/authentication/adapters/electron/live-notification-service";
import { twitchDeviceAuthWindow } from "@backend/features/authentication/adapters/electron/twitch-device-auth-window";
import { kickFollowWriteService } from "@backend/features/authentication/adapters/kick/kick-follow-write-service";
import {
  runTwitchDeviceCodeLogin,
  type TwitchDeviceCodeLoginDependencies,
} from "@backend/features/authentication/adapters/twitch/device-code-flow";
import {
  authWindowManager,
  ...
  twitchAuthService,
  validateOAuthConfig,
} from "@backend/features/authentication/composition/auth-runtime";
...
import {
  ...
  resolveAuthStatus,
} from "@backend/features/authentication/domain/auth-status";
import { beginKickAccountReconciliation } from "@backend/features/authentication/domain/kick-account-reconciliation-coordinator";
import { disposeSendWindow } from "@backend/features/chat/adapters/kick/kick-send-window";
import type { FollowedChannelsResult } from "@backend/features/authentication/adapters/kick/follow-endpoints";
...
import { IPC_CHANNELS, type AuthStatus, type AuthSyncFollowsResult } from "@shared/ipc-channels";
...
export async function syncKickFollowsAfterLogin(
  ...
  storage: Pick<typeof authenticationRepository, "upsertSyncedFollows"> &
    Partial<
      Pick<typeof authenticationRepository, "getLocalFollowsByPlatform" | "getKickUser"> &
        Pick<
...
export async function syncTwitchFollowsAfterLogin(
  ...
  storage: Pick<typeof authenticationRepository, "upsertSyncedFollows"> = authenticationRepository
...
export function persistInitialAuthToken(
  platform: Platform,
  token: AuthToken,
  storage: Pick<typeof authenticationRepository, "saveToken"> &
    Partial<
      Pick<typeof authenticationRepository, "invalidateKickAccountFollows">
    > = authenticationRepository
): AuthToken {
  if (platform === "kick") {
...
export function registerAuthHandlers(
  ...
  const fetchMissingTwitchUserOnce = createTwitchIdentitySingleFlight(
    () => authenticationRepository.getToken("twitch")?.accessToken ?? null,
    ...
    try {
      ...
      if (platform === "twitch") {
        ...
        const { getKickAccountFollowState, getKickAccountFollowStates } =
          await import("@backend/features/discovery/adapters/kick/kick-public-profile-reader");
  ...
  ipcMain.handle(IPC_CHANNELS.AUTH_GET_TOKEN, (event, payload: unknown) => {
    ...
    return authenticationRepository.getToken(parsed.data.platform);
  });
  ...
  ipcMain.handle(IPC_CHANNELS.AUTH_CLEAR_TOKEN, (event, payload: unknown) => {
    ...
    if (!parsed.success) return;
    authenticationRepository.clearToken(parsed.data.platform);
  ...
  ipcMain.handle(IPC_CHANNELS.AUTH_HAS_TOKEN, (event, payload: unknown) => {
    ...
    return authenticationRepository.hasToken(parsed.data.platform);
  });
  ...
  ipcMain.handle(IPC_CHANNELS.AUTH_IS_TOKEN_EXPIRED, (event, payload: unknown) => {
    ...
    return authenticationRepository.isTokenExpired(parsed.data.platform);
  });
  ...
  ipcMain.handle(IPC_CHANNELS.AUTH_GET_TWITCH_USER, (event) => {
    if (!isAllowedSender(event)) return null;
    return authenticationRepository.getTwitchUser();
  });
  ...
  ipcMain.handle(IPC_CHANNELS.AUTH_SAVE_TWITCH_USER, (event, payload: unknown) => {
    ...
    if (!parsed.success) return;
    authenticationRepository.saveTwitchUser(parsed.data.user);
  });

  ipcMain.handle(IPC_CHANNELS.AUTH_CLEAR_TWITCH_USER, (event) => {
    if (!isAllowedSender(event)) return;
    authenticationRepository.clearTwitchUser();
  });

  ipcMain.handle(IPC_CHANNELS.AUTH_GET_KICK_USER, (event) => {
    if (!isAllowedSender(event)) return null;
    return authenticationRepository.getKickUser();
  });
  ...
  ipcMain.handle(IPC_CHANNELS.AUTH_SAVE_KICK_USER, (event, payload: unknown) => {
    ...
    if (!parsed.success) return;
    authenticationRepository.saveKickUser(parsed.data.user);
  });

  ipcMain.handle(IPC_CHANNELS.AUTH_CLEAR_KICK_USER, (event) => {
    if (!isAllowedSender(event)) return;
    authenticationRepository.clearKickUser();
  });
  ...
  ipcMain.handle(IPC_CHANNELS.AUTH_GET_STATUS, async (event): Promise<AuthStatus> => {
    ...
    return await resolveAuthStatus({
      getTwitchUser: () => authenticationRepository.getTwitchUser(),
      getTwitchToken: () => authenticationRepository.getToken("twitch"),
      getKickUser: () => authenticationRepository.getKickUser(),
      hasUsableToken: (platform) => authenticationRepository.hasUsableToken(platform),
      isTokenExpired: (platform) => authenticationRepository.isTokenExpired(platform),
  ...
  async function handleKickOAuthFlow(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        if (user) {
          authenticationRepository.saveKickUser(user);
        }
      } catch (userError) {
    ...
    const login = (async (): Promise<{ success: boolean; error?: string }> => {
      ...
      try {
        ...
        await performTwitchDeviceCodeLogin({
          ...
          saveToken: (platform, token) => authenticationRepository.saveToken(platform, token),
          scheduleProactiveRefresh: () => twitchAuthService.scheduleProactiveRefresh(),
          fetchCurrentUser: () => {
            const token = authenticationRepository.getToken("twitch");
            return token
              ? twitchAuthService.fetchCurrentUser(token.accessToken)
              : Promise.resolve(null);
          },
          clearTwitchUser: () => authenticationRepository.clearTwitchUser(),
          saveTwitchUser: (user) => authenticationRepository.saveTwitchUser(user),
  ...
  ipcMain.handle(IPC_CHANNELS.AUTH_REFRESH_TWITCH, async (event) => {
    ...
    try {
      ...
      if (token) {
        ...
        return {
          success: true,
          user: authenticationRepository.getTwitchUser(),
          hasToken: authenticationRepository.hasToken("twitch"),
          isExpired: authenticationRepository.isTokenExpired("twitch"),
        };
      }
