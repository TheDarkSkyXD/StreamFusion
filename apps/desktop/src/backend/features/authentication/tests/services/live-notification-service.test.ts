...
} from "@backend/features/authentication/adapters/electron/live-notification-service";
import {
...
describe("LiveNotificationService", () => {
  it("includes signed-out Guest Follows when no platform-account rows are active", () => {
    ...
    const getActive = vi
      .spyOn(authenticationRepository, "getActiveFollowsByPlatform")
      .mockImplementation(() => []);
    const getGuest = vi
      .spyOn(authenticationRepository, "getGuestFollowsByPlatform")
