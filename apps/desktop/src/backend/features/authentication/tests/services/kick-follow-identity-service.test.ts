...
} from "@backend/features/authentication/adapters/kick/kick-follow-identity-service";
...
describe("Kick follow identity resolution and verification metadata", () => {
  it("repairs a slug-keyed follow through the broadcaster ID in its avatar URL", async () => {
    ...
    vi.mocked(authenticationRepository.getLocalFollowsByPlatform).mockReturnValue([staleFollow]);
  ...
  it("hydrates an ID-less follow by slug without downgrading stored display-name casing", async () => {
    ...
    vi.mocked(authenticationRepository.getLocalFollowsByPlatform).mockReturnValue([legacyFollow]);
    ...
    expect(authenticationRepository.updateLocalFollow).toHaveBeenCalledWith("row-nickwhite", {
      channelId: "123",
    });
  });
  ...
  it("does not rewrite metadata when a stale caller is behind the stored follow", async () => {
    ...
    vi.mocked(authenticationRepository.getLocalFollowsByPlatform).mockReturnValue([storedCurrent]);
    const client = {
    ...
    expect(authenticationRepository.updateLocalFollow).not.toHaveBeenCalled();
  ...
  it("treats medium and fullsize renditions of the same Kick avatar as equivalent", async () => {
    ...
    vi.mocked(authenticationRepository.getLocalFollowsByPlatform).mockReturnValue([storedFollow]);
    const client = {
      getChannelsByBroadcasterIds: vi.fn().mockResolvedValue([currentChannel]),
      getPublicChannel: vi.fn().mockResolvedValue({
        ...currentChannel,
  ...
  it("persists a genuinely new Kick avatar asset", async () => {
    ...
    expect(authenticationRepository.updateLocalFollow).toHaveBeenCalledWith(storedFollow.id, {
  ...
  it("resolves playback for a renamed slug-keyed follow through its avatar identity", async () => {
    ...
    vi.mocked(authenticationRepository.getActiveFollowsByPlatform).mockReturnValue([staleFollow]);
    vi.mocked(authenticationRepository.getLocalFollowsByPlatform).mockReturnValue([staleFollow]);
    const renamedChannel = makeChannel({
      id: "110821336",
      username: "abbyapple",
  ...
  it("persists verified metadata by stable broadcaster ID and reuses it after the slug is current", async () => {
    ...
    vi.mocked(authenticationRepository.getLocalFollowsByPlatform).mockReturnValue([currentFollow]);
  ...
  it("serializes concurrent cache updates so entries and rotation progress are not lost", async () => {
    ...
    vi.mocked(authenticationRepository.getLocalFollowsByPlatform).mockReturnValue(allFollows);
  ...
  it("revalidates stale verified metadata so a revoked badge is eventually removed", async () => {
    ...
    try {
      ...
      vi.mocked(authenticationRepository.getLocalFollowsByPlatform).mockReturnValue([follow]);
      const currentChannel = makeChannel();
