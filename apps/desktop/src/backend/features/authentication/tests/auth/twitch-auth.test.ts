...
describe("logout", () => {
  it("revokes token, clears storage, and cancels proactive refresh", async () => {
    ...
    expect(authenticationRepository.clearToken).toHaveBeenCalledWith("twitch");
    expect(authenticationRepository.clearTwitchUser).toHaveBeenCalled();
  });
  ...
  it("succeeds even when no token is stored (nothing to revoke)", async () => {
    ...
    expect(authenticationRepository.clearToken).toHaveBeenCalled();
  });
});
...
describe("fetchCurrentUser", () => {
  ...
  it("fetches and transforms user data from Twitch API", async () => {
    ...
    expect(authenticationRepository.saveTwitchUser).toHaveBeenCalled();
  });
  ...
  it("does not save a fetched user when the Twitch token rotated during the request", async () => {
    ...
    expect(authenticationRepository.saveTwitchUser).not.toHaveBeenCalled();
