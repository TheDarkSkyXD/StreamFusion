...
const { kickAuthService } =
  await import("@backend/features/authentication/adapters/kick/kick-auth");
...
describe("refreshToken", () => {
  ...
  it("refreshes and saves new token on success", async () => {
    ...
    expect(authenticationRepository.saveToken).toHaveBeenCalledWith("kick", {
      ...newToken,
      scope: canonicalScopes,
    });
  });
  ...
  it("rejects and does not persist an explicitly incomplete refreshed scope set", async () => {
    ...
    expect(authenticationRepository.saveToken).not.toHaveBeenCalled();
    expect(authenticationRepository.clearToken).not.toHaveBeenCalled();
  });
  ...
  it("preserves auth state and web cookies on transient refresh failure", async () => {
    ...
    expect(result).toBeNull();
    expect(authenticationRepository.clearToken).not.toHaveBeenCalled();
    expect(authenticationRepository.clearKickUser).not.toHaveBeenCalled();
  ...
  it("invalidates only OAuth on permanent rejection and preserves website auth", async () => {
    ...
    expect(authenticationRepository.clearToken).toHaveBeenCalledWith("kick");
    expect(authenticationRepository.clearKickUser).not.toHaveBeenCalled();
    expect(authenticationRepository.clearKickWebBearer).not.toHaveBeenCalled();
    expect(mockCookiesRemove).not.toHaveBeenCalled();
...
describe("proactive refresh", () => {
  it("rotates the token five minutes before expiry and schedules the next rotation", async () => {
    ...
    expect(authenticationRepository.saveToken).toHaveBeenCalledWith(
      "kick",
...
describe("logout", () => {
  it("clears token, user, and session cookies", async () => {
    ...
    expect(authenticationRepository.clearToken).toHaveBeenCalledWith("kick");
    expect(authenticationRepository.clearKickWebBearer).toHaveBeenCalledOnce();
    expect(authenticationRepository.clearKickUser).toHaveBeenCalled();
  });
...
describe("fetchCurrentUser", () => {
  ...
  it("fetches, transforms, and saves user from Kick API", async () => {
    ...
    expect(authenticationRepository.saveKickUser).toHaveBeenCalled();
  });
