...
import { TokenRefreshError } from "@backend/features/authentication/adapters/oauth/token-exchange";
...
// Guards: Twitch token-refresh flow — 401 → refresh → retry, refresh-failure → clearToken + signed-out state, expired-token detection. Module-level mocks for authenticationRepository and tokenExchangeService keep the refresh-decision logic isolated from disk/network so the chain can be driven deterministically. Drift on the singleton's lookup-then-decide ordering (e.g. caching a stale token in memory across a refresh) surfaces here.

// The service singleton imports authenticationRepository and tokenExchangeService at
...
vi.mock("@backend/features/authentication/adapters/oauth/token-exchange", async () => {
  const actual = await vi.importActual<
    typeof import("@backend/features/authentication/adapters/oauth/token-exchange")
  >("@backend/features/authentication/adapters/oauth/token-exchange");
  return {
    ...actual,
...
const { tokenExchangeService } =
  await import("@backend/features/authentication/adapters/oauth/token-exchange");
...
describe("twitchAuthService refresh chain", () => {
  it("preserves an older partial scope set when Twitch omits scopes on refresh", async () => {
    ...
    expect(storageModule.authenticationRepository.saveToken).toHaveBeenCalledWith(
      "twitch",
  ...
  it("retries with exponential backoff on transient failure", async () => {
    ...
    await vi.advanceTimersByTimeAsync(55 * 60 * 1000 + 1000);
    expect(refreshTokenMock).toHaveBeenCalledTimes(1);
    expect(storageModule.authenticationRepository.clearToken).not.toHaveBeenCalled();
    ...
    expect(storageModule.authenticationRepository.saveToken).toHaveBeenCalled();
    twitchAuthService.cancelProactiveRefresh();
  });
  ...
  it("invalidates auth immediately on a permanent failure (invalid_grant) but keeps stored TwitchUser", async () => {
    ...
    expect(storageModule.authenticationRepository.clearToken).toHaveBeenCalledWith("twitch");
    ...
    expect(storageModule.authenticationRepository.clearTwitchUser).not.toHaveBeenCalled();
