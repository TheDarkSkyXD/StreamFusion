...
import { registerTokenStatusHandlers } from "@backend/features/authentication/routes/token-status-routes";
...
describe("AUTH_TOKEN_STATUS", () => {
  ...
  it("returns full token status report when token exists and validates", async () => {
    ...
    vi.mocked(authenticationRepository.getToken).mockReturnValue(fakeToken);
  ...
  it("returns valid=false when getTokenStatus reports invalid", async () => {
    vi.mocked(isAllowedSender).mockReturnValue(true);
    vi.mocked(authenticationRepository.hasToken).mockReturnValue(true);
    vi.mocked(authenticationRepository.getToken).mockReturnValue({ accessToken: "x" });
