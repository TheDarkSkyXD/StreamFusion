...
export function registerSystemHandlers(renderer: MainRendererPort): void {
  ...
  ipcMain.handle(
    ...
    (_event, { title, body }: { title: string; body: string }) => {
      const preferences = preferencesRepository.getPreferences().notifications;
  ...
  ipcMain.handle(IPC_CHANNELS.NOTIFICATION_COVERAGE_GET, async () => {
    const { liveNotificationService } =
      await import("@backend/features/authentication/adapters/electron/live-notification-service");
