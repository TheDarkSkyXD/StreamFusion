...
const fallbackSnapshots = new Map<
  string,
  { expiresAt: number; promise: ReturnType<typeof getPublicChannelUserProfile> }
>();
...
function readFallback(
  ...
  const promise = getPublicChannelUserProfile(channelSlug, username);
...
export async function getKickPublicIdentity(
  ...
  if (numericUserId !== null) {
    try {
      const user = (await getUsersByIdStrict(kickTransport, [numericUserId])).find(
...
export async function getKickAccountFollowStates(
  ...
  const profiles = await getPublicChannelUserProfiles(
...
export async function resolveKickPublicChannel(
  ...
  try {
    const channel = (await getChannelsBySlugs(kickTransport, [username])).find(
