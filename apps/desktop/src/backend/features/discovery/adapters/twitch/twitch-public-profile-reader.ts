...
import { gqlGetChannelByLogin } from "@backend/features/discovery/adapters/twitch/twitch-gql-discovery";
import { getUsersById, getUsersByLogin } from "./user-endpoints";
import { helixResponseSchema } from "../../../../api/platforms/twitch/twitch-helix-schemas";
...
export async function getTwitchPublicIdentity(
  ...
  if (twitchAuthService.isAuthenticated()) {
    try {
      const user = (await getUsersById(twitchTransport, [userId]))[0];
      if (user) {
        return {
          state: "known",
          source: "official",
...
export async function getTwitchFollowRelationship(
  ...
  try {
    ...
    const response = helixResponseSchema(z.object({ followed_at: z.string() })).parse(
      await twitchTransport.request(`/channels/followers?${query.toString()}`)
    );
...
export async function resolveTwitchPublicChannel(
  ...
  if (twitchAuthService.isAuthenticated()) {
    try {
      const user = (await getUsersByLogin(twitchTransport, [username]))[0];
      if (user) {
        return {
          state: "known",
          source: "official",
  ...
  try {
    const channel = await gqlGetChannelByLogin(username);
    return channel
      ? {
          state: "known",
          source: "first-party-fallback",
