...
import { filterRankAndDeduplicateVideos } from "@backend/features/discovery/domain/search-match-contract";
...
vi.mock("@backend/features/discovery/composition/twitch-discovery", () => ({
  twitchDiscovery: clients.twitch,
}));
vi.mock("@backend/features/authentication/composition/twitch-account-reader", () => ({
  twitchAccountReader: clients.twitch,
}));
vi.mock("@backend/features/playback/composition/twitch-playback", () => ({
  twitchPlayback: clients.twitch,
}));
vi.mock("@backend/api/platforms/twitch/twitch-transport", () => ({
  twitchTransport: clients.twitch,
}));
vi.mock("@backend/features/discovery/composition/kick-discovery", () => ({
  kickDiscovery: clients.kick,
}));
vi.mock("@backend/features/authentication/composition/kick-account-reader", () => ({
  kickAccountReader: clients.kick,
}));
vi.mock("@backend/features/playback/composition/kick-playback", () => ({
  kickPlayback: clients.kick,
}));
vi.mock("@backend/api/platforms/kick/kick-transport", () => ({ kickTransport: clients.kick }));

import { createFocusedRecentContentSources } from "@backend/features/discovery/domain/focused-search-sources";

const focusedRecentContentSources = createFocusedRecentContentSources(clients, clients);
