...
vi.mock("@backend/features/discovery/composition/kick-discovery", () => ({ kickDiscovery: mocks }));
vi.mock("@backend/features/authentication/composition/kick-account-reader", () => ({
  kickAccountReader: mocks,
}));
vi.mock("@backend/features/playback/composition/kick-playback", () => ({ kickPlayback: mocks }));
vi.mock("@backend/api/platforms/kick/kick-transport", () => ({ kickTransport: mocks }));
...
} from "@backend/features/discovery/adapters/kick/kick-public-profile-reader";
...
vi.mock("@backend/features/discovery/adapters/kick/user-endpoints", () => ({
  getPublicChannelUserProfile: mocks.getPublicChannelUserProfile,
  getPublicChannelUserProfiles: mocks.getPublicChannelUserProfiles,
  getUsersByIdStrict: (_transport: unknown, ids: number[]) => mocks.getUsersByIdStrict(ids),
}));
vi.mock("@backend/features/discovery/adapters/kick/channel-endpoints", () => ({
  getChannelsBySlugs: (_transport: unknown, slugs: string[]) => mocks.getChannelsBySlugs(slugs),
}));
