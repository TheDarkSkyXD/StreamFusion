...
vi.mock("@backend/features/authentication/data/authentication-repository", () => ({
  authenticationRepository: { getToken: mocks.getToken },
}));
...
vi.mock("@backend/api/platforms/twitch/twitch-transport", () => ({
  twitchTransport: { request: mocks.request },
}));
...
} from "@backend/features/discovery/adapters/twitch/twitch-public-profile-reader";
...
vi.mock("@backend/features/authentication/adapters/twitch/twitch-auth", () => ({
  twitchAuthService: { isAuthenticated: mocks.isAuthenticated },
}));
vi.mock("@backend/features/discovery/adapters/twitch/twitch-gql-discovery", () => ({
  gqlGetChannelByLogin: mocks.getChannelByLogin,
}));
vi.mock("@backend/features/discovery/adapters/twitch/user-endpoints", () => ({
  getUsersById: (_transport: unknown, ids: string[]) => mocks.getUsersById(ids),
  getUsersByLogin: (_transport: unknown, logins: string[]) => mocks.getUsersByLogin(logins),
}));
