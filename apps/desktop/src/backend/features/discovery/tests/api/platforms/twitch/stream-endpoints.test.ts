...
import {
  ...
  getStreamsByUserIds,
  getTopStreams,
} from "@backend/features/discovery/adapters/twitch/stream-endpoints";
...
// Guards: primary stream failures remain errors; only a successful empty response means no live streams.
// Guards: requested IDs, page size and cursor survive the primary Helix request and response.
describe("getStreamsByUserIds", () => {
  ...
  it("propagates a failed primary page instead of reporting every channel offline", async () => {
    const failure = new Error("Twitch HTTP 503: unavailable");
    const client = makeClient({});
    vi.mocked(client.request).mockRejectedValueOnce(failure);

    await expect(
      getStreamsByUserIds(client, ["u1"], { first: 25, after: "page-two" })
    ).rejects.toBe(failure);
    expect(client.request).toHaveBeenCalledExactlyOnceWith(
      "/streams?first=25&user_id=u1&after=page-two"
    );
  });

  it("preserves a successful empty provider response without requesting user enrichment", async () => {
    const client = makeClient({ "/streams": { data: [], pagination: {} } });

    await expect(getStreamsByUserIds(client, ["u1"])).resolves.toEqual({
      data: [],
      cursor: undefined,
    });
    expect(client.request).toHaveBeenCalledExactlyOnceWith("/streams?first=100&user_id=u1");
  });

  it("propagates failed user enrichment rather than fabricating an empty live page", async () => {
    const failure = new Error("User metadata unavailable");
    const client = makeClient({});
    vi.mocked(client.request)
      .mockResolvedValueOnce({ data: [STREAM], pagination: { cursor: "next" } })
      .mockRejectedValueOnce(failure);

    await expect(getStreamsByUserIds(client, ["u1"])).rejects.toBe(failure);
    expect(client.request).toHaveBeenNthCalledWith(2, "/users?id=u1");
  });
