...
describe("getPublicStreamBySlug — circuit-open (slice 03)", () => {
  let getPublicStreamBySlug: typeof import("@backend/features/discovery/adapters/kick/stream-endpoints").getPublicStreamBySlug;
  let __resetCircuitProbeForTests: typeof import("@backend/features/discovery/adapters/kick/stream-endpoints").__resetCircuitProbeForTests;
  ...
  beforeEach(async () => {
    ...
    ({ getPublicStreamBySlug, __resetCircuitProbeForTests } = await import(
      "@backend/features/discovery/adapters/kick/stream-endpoints"
    ));
