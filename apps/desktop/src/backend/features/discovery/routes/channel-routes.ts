...
import { dedupeChannelsByIdentity } from "@shared/utils/stream-identity";
...
import type { KickFollowMetadataClient } from "../../authentication/adapters/kick/kick-follow-identity-service";
import { buildKickFollowedChannelSnapshot } from "../../authentication/adapters/kick/kick-follow-identity-service";

interface TwitchChannelReader
  extends ChannelReader<"twitch", UnifiedChannel, ChannelRef> {
...
export interface ChannelHandlerDependencies {
  readonly twitchFollows: FollowedChannelReader<"twitch", UnifiedChannel>;
  readonly readers: {
...
function refreshStoredFollowFromResolvedChannel(
  ...
  const follow = authenticationRepository
    .getActiveFollowsByPlatform(channel.platform)
    .find((candidate) => {
  ...
  authenticationRepository.updateLocalFollow(follow.id, updates);
  if (channel.platform === "kick" && follow.source === "kick") {
    authenticationRepository.upsertSyncedFollows(
      "kick",
      [
        {
          platform: "kick",
...
export function registerChannelHandlers({ readers, twitchFollows }: ChannelHandlerDependencies): void {
  /**
   * Get channel by ID
   */
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        if (params.platform === "twitch") {
          ...
          if (!channel) {
            const staleFollow = authenticationRepository
          ...
          if (authoritativeNotFoundFollowId && channel?.accountStatus !== "suspended") {
            authenticationRepository.removeLocalFollow(authoritativeNotFoundFollowId);
            channel = null;
          }
          if (channel && !channel.accountStatus) {
            channel = { ...channel, accountStatus: "active" };
          }
          channel = enrichOwnKickChannel(
            channel,
            params.username,
            authenticationRepository.getKickUser()
          );
        }
      ...
      try {
        ...
        if (params.platform === "twitch") {
          if (readers.twitch.isAuthenticated()) {
            ...
            channels = await twitchFollows.getAllFollowedChannels();
          }
        } else if (params.platform === "kick") {
          const follows = authenticationRepository.getActiveFollowsByPlatform("kick");
