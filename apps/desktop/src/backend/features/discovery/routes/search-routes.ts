...
import {
  ...
  createFocusedStreamSources,
} from "@backend/features/discovery/domain/focused-search-sources";
import { createProgressiveClipSearch } from "@backend/features/discovery/domain/progressive-clip-search";
import {
  createProgressiveStreamSearch,
  readStreamSearchFailureProgress,
  STREAM_SEARCH_BUDGET_PROFILES,
} from "@backend/features/discovery/domain/progressive-stream-search";
import { createProgressiveVideoSearch } from "@backend/features/discovery/domain/progressive-video-search";
import {
  attachSearchSession,
  cancelSearchSession,
  isSearchCancelled,
} from "@backend/features/discovery/domain/search-session-manager";
...
export interface SearchHandlerDependencies {
  readonly contentReaders: Readonly<Record<Platform,
...
export function registerSearchHandlers({ readers, contentReaders }: SearchHandlerDependencies): void {
  const activeBroadSearches = new Map<string, AbortController>();
  const focusedRecentContentSources = createFocusedRecentContentSources(readers, contentReaders);
  const focusedStreamSources = createFocusedStreamSources(readers);
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        const kickUser = authenticationRepository.getKickUser();
        const twitchUser = authenticationRepository.getTwitchUser();
