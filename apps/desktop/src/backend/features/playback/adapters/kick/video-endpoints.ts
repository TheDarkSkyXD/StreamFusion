...
import { isKickRequestCancellation } from "../../../../api/platforms/kick/kick-error-classification";
import { normalizeKickDate } from "../../../../api/platforms/kick/kick-transformers";
import {
    ...
    return {
      data: videos
        ...
        .map((v) => {
          ...
          const recordedCategory = v.categories?.[0] ?? v.category ?? v.livestream?.categories?.[0];
          ...
          return {
            ...
            channelAvatar:
              ...
              null,
            gameId: recordedCategory?.id === undefined ? "" : String(recordedCategory.id),
            category: recordedCategory?.name || "",
            // Language info
