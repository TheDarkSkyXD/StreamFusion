...
describe("video-endpoints — getVideosByChannelSlug", () => {
  let getVideosByChannelSlug: typeof import("@backend/features/playback/adapters/kick/video-endpoints").getVideosByChannelSlug;
  let getLatestCompletedVideoEndedAtByChannelSlug: typeof import("@backend/features/playback/adapters/kick/video-endpoints").getLatestCompletedVideoEndedAtByChannelSlug;
  ...
  beforeEach(async () => {
    ...
    ({ getVideosByChannelSlug, getLatestCompletedVideoEndedAtByChannelSlug } =
      await import("@backend/features/playback/adapters/kick/video-endpoints"));
  });
  ...
  it("keeps curated guest category videos tied to recorded category metadata", async () => {
    const { KickPlayback } =
      await import("@backend/features/playback/adapters/kick/kick-playback-reader");
    const getStreamsByCategory = vi.fn(async () => ({
      data: [
        {
          id: "live",
          ...
          title: "Now chatting",
          viewerCount: 10,
          thumbnailUrl: "https://example.com/live.png",
          isLive: true,
    ...
    mockNetFetch.mockResolvedValueOnce(
      jsonResponse([
        { ...common, id: 1, categories: [{ id: 15, name: "Renamed category" }] },
        { ...common, id: 2, categories: [{ id: 99, name: "Just Chatting" }] },
        { ...common, id: 3, categories: [{ name: "Just Chatting" }] },
        { ...common, id: 4 },
        { ...common, id: 5, categories: [{ name: "Minecraft" }] },
        { ...common, id: 6, livestream: { session_title: "Just Chatting" } },
      ])
    );
    const page = await new KickPlayback({ getStreamsByCategory }).readCategoryVideos({
      id: "15",
      name: "Just Chatting",
    });
    expect(page.kind).toBe("available");
    if (page.kind !== "available") throw new Error("expected category videos");
    expect(page.data.map((video) => video.id).sort()).toEqual(["1", "3"]);
    expect(page.data.find((video) => video.id === "1")).toMatchObject({
      categoryId: "15",
      categoryName: "Renamed category",
    });
    expect(page.data.find((video) => video.id === "3")).toMatchObject({
      categoryId: "",
      categoryName: "Just Chatting",
    });
    expect(mockNetFetch).toHaveBeenCalledTimes(1);
    expect(mockNetFetch.mock.calls[0][0]).toContain("/channels/streamer/videos?");
    expect(mockNetFetch.mock.calls[0][1]).toMatchObject({
