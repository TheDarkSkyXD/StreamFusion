...
vi.mock("@backend/features/playback/adapters/electron/network-adblock-service", () => ({
...
vi.mock("@backend/features/playback/adapters/electron/cosmetic-injection-service", () => ({
...
vi.mock("@backend/features/playback/adapters/twitch/twitch-manifest-proxy", () => ({
  twitchManifestProxy: {
    isActive: vi.fn(),
    getStats: vi.fn(),
...
vi.mock("@backend/features/playback/data/vaft-pattern-service", () => ({
  vaftPatternService: {
...
import { registerAdBlockHandlers } from "@backend/features/playback/routes/adblock-routes";
