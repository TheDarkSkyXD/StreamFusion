...
vi.mock("@backend/features/discovery/composition/kick-discovery", () => ({
  kickDiscovery: { platform: "kick" as const, getStreamsByCategory: vi.fn() },
}));
...
vi.mock("@backend/features/discovery/composition/twitch-discovery", () => ({
  twitchDiscovery: { platform: "twitch" as const, getUsersById: vi.fn(), getTopStreams: vi.fn() },
}));
...
import { KickStreamResolver } from "@backend/features/playback/adapters/kick/kick-stream-resolver";
import { logger } from "@backend/logging/logger";
import { twitchPlayback } from "@backend/features/playback/composition/twitch-playback";
import { TwitchStreamResolver } from "@backend/features/playback/adapters/twitch/twitch-stream-resolver";
import {
  fillPageWithCutoff,
  getCutoffMs,
  handleGetClipsByChannel,
  registerVideoHandlers,
} from "@backend/features/playback/routes/video-routes";

const DAY_MS = 24 * 60 * 60 * 1000;
const FROZEN_NOW = new Date("2026-06-06T12:00:00.000Z").getTime();

const getClipsMock = vi.mocked(kickPlayback.getClips);
const getClipsByCategoryMock = vi.mocked(kickPlayback.getClipsByCategory);
const getVideosMock = vi.mocked(kickPlayback.getVideos);
const getStreamsByCategoryMock = vi.mocked(kickDiscovery.getStreamsByCategory);
const twitchGetClipsByChannelMock = vi.mocked(twitchPlayback.getClipsByChannel);
const twitchGetClipsByGameMock = vi.mocked(twitchPlayback.getClipsByGame);
const twitchGetVideosByGameMock = vi.mocked(twitchPlayback.getVideosByGame);
const twitchGetVideosByChannelMock = vi.mocked(twitchPlayback.getVideosByChannel);
const twitchGetVideosByUserMock = vi.mocked(twitchPlayback.getVideosByUser);
const twitchGetTopStreamsMock = vi.mocked(twitchDiscovery.getTopStreams);
const twitchGetUsersByIdMock = vi.mocked(twitchDiscovery.getUsersById);
const kickReadCategoryClipsMock = vi.mocked(kickPlayback.readCategoryClips);
const kickReadCategoryVideosMock = vi.mocked(kickPlayback.readCategoryVideos);
const twitchReadCategoryClipsMock = vi.mocked(twitchPlayback.readCategoryClips);
const twitchReadCategoryVideosMock = vi.mocked(twitchPlayback.readCategoryVideos);
type ClipAgeRow = readonly [id: string, ageMs: number];
type KickClip = Awaited<ReturnType<typeof kickPlayback.getClips>>["data"][number];
type TwitchClip = Awaited<ReturnType<typeof twitchPlayback.getClipsByChannel>>["data"][number];
type KickCategoryClip = Awaited<ReturnType<typeof kickPlayback.getClipsByCategory>>["data"][number];
type KickVideo = Awaited<ReturnType<typeof kickPlayback.getVideos>>["data"][number];
...
describe("IPC handlers - VIDEOS_GET_METADATA", () => {
  beforeEach(() => {
    vi.useRealTimers();
    vi.mocked(ipcMain.handle).mockReset();
    vi.mocked(twitchPlayback.getVideoById).mockReset();
    kickResolverProto.getVideoMetadata.mockReset();
    registerHandlers();
  });

  it("returns formatted Twitch video metadata", async () => {
    vi.mocked(twitchPlayback.getVideoById).mockResolvedValue(unifiedVideo());
  ...
  it("returns error when Twitch video not found", async () => {
    vi.mocked(twitchPlayback.getVideoById).mockResolvedValue(null);
...
describe("IPC handlers - VIDEOS_GET_BY_CHANNEL", () => {
  beforeEach(() => {
    vi.useRealTimers();
    vi.mocked(ipcMain.handle).mockReset();
    vi.mocked(twitchPlayback.getVideosByChannel).mockReset();
    vi.mocked(twitchPlayback.getVideosGameData).mockReset();
    vi.mocked(kickPlayback.getVideos).mockReset();
    registerHandlers();
  });

  it("returns mapped Twitch videos with game data", async () => {
    vi.mocked(twitchPlayback.getVideosByChannel).mockResolvedValue({
      data: [unifiedVideo({ id: "v1", title: "Stream 1", duration: 7200, viewCount: 1000 })],
      cursor: "vc",
    });
    vi.mocked(twitchPlayback.getVideosGameData).mockResolvedValue({
  ...
  it("lowercases Twitch channel login for GQL", async () => {
    ...
    expect(twitchPlayback.getVideosByChannel).toHaveBeenCalledWith("mychannel", expect.anything());
  });
  ...
  it("returns Kick videos with client-side view sort", async () => {
    vi.mocked(kickPlayback.getVideos).mockResolvedValue({
...
describe("IPC handlers - VIDEOS_GET_BY_LIVESTREAM_ID", () => {
  ...
  it("paginates through multiple pages to find the VOD", async () => {
    vi.mocked(kickPlayback.getVideos)
      .mockResolvedValueOnce({
    ...
    expectSuccessful(result);
    expect(result.data.id).toBe("v2");
    expect(kickPlayback.getVideos).toHaveBeenCalledTimes(2);
  });
  ...
  it("stops after maxAttempts (5 pages) to prevent infinite loops", async () => {
    ...
    expect(result.success).toBe(false);
    expect(kickPlayback.getVideos).toHaveBeenCalledTimes(5);
  });

  it("returns error on API failure", async () => {
    vi.mocked(kickPlayback.getVideos).mockRejectedValue(new Error("network error"));
