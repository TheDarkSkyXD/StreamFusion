...
import { KickStreamResolver } from "../adapters/kick/kick-stream-resolver";
import { TwitchStreamResolver } from "../adapters/twitch/twitch-stream-resolver";
import type { UnifiedClip, UnifiedVideo } from "../../../../shared/platform-types";

type KickClip = Awaited<
  ReturnType<typeof import("../adapters/kick/clip-endpoints").getClipsByChannelSlug>
>["data"][number];
type KickVideo = Awaited<
  ReturnType<typeof import("../adapters/kick/video-endpoints").getVideosByChannelSlug>
>["data"][number];
type RecordedContentReader = Pick<
  VideoReader<Platform, UnifiedVideo, unknown, AbortSignal>,
  "platform" | "readCategoryVideos"
> &
  Pick<ClipReader<Platform, UnifiedClip, unknown, AbortSignal>, "readCategoryClips">;

export interface VideoHandlerDependencies {
  readonly readers: Readonly<Record<Platform, RecordedContentReader>>;
}
...
function toCategoryMediaItem(
  item: UnifiedVideo | UnifiedClip,
  request: CategoryVideosRequest | CategoryClipsRequest
): CategoryMediaItem {
  const isClip = "clipUrl" in item;
  const publishedAt = isClip ? item.createdAt : item.publishedAt;
  const categoryId = item.categoryId || (isClip ? item.gameId : undefined) || request.categoryId;
  const categoryName =
    item.categoryName || (isClip ? item.gameName : undefined) || request.categoryName || "";
  return {
    id: item.id,
    title: item.title,
    ...
    creatorName: isClip ? item.creatorName : undefined,
    embedUrl: isClip ? item.embedUrl : undefined,
    url: isClip ? item.clipUrl : item.url,
    shareUrl: item.shareUrl,
    source: isClip ? undefined : item.source,
    isLive: isClip ? undefined : item.isLive,
    isSubOnly: isClip ? undefined : item.isSubOnly,
    language: item.language,
    vodId: isClip ? item.vodId : undefined,
  };
}

function toCategoryMediaResult(
  result: CategoryContentResult<UnifiedVideo | UnifiedClip>,
  request: CategoryVideosRequest | CategoryClipsRequest
): CategoryMediaResult {
  ...
  return {
    ...
    data: result.data.map((item) => toCategoryMediaItem(item, request)),
...
export async function handleGetClipsByChannel(params: ClipsGetByChannelParams) {
  const { twitchPlayback } = await import("@backend/features/playback/composition/twitch-playback");
  const { kickPlayback } = await import("@backend/features/playback/composition/kick-playback");
    ...
    if (params.platform === "twitch") {
        ...
        if (initialDateCandidateFilters.length > 0) {
          ...
          const pages = await Promise.all(
            initialDateCandidateFilters.map((filter) =>
              twitchPlayback.getClipsByChannel(channelLogin, {
          ...
          if (twitchClips.length === 0 && effectiveGqlFilter !== gqlFilter) {
            const fallback = await twitchPlayback.getClipsByChannel(channelLogin, {
              first: gqlFirst,
              filter: gqlFilter,
            });
          ...
          let clips = await twitchPlayback.getClipsByChannel(channelLogin, {
            first: gqlFirst,
            after: params.cursor,
            filter: effectiveGqlFilter,
          });
          if (clips.data.length === 0 && effectiveGqlFilter !== gqlFilter) {
            clips = await twitchPlayback.getClipsByChannel(channelLogin, {
              first: gqlFirst,
              after: params.cursor,
              filter: gqlFilter,
            });
        ...
        if (initialDateCandidateFilters.length > 0) {
          ...
          const result = await fillPageWithCutoff<UnifiedClip>({
            ...
            fetchPage: async (cursor) => {
              const clips = await twitchPlayback.getClipsByChannel(channelLogin, {
      ...
      if (isViewSortWithTimeParams) {
        ...
        while (keepFetching && pagesFetched < MAX_PAGES) {
          ...
          const response = await kickPlayback.getClips(params.channelName, {
            limit: 100,
      ...
      if (clipsToCheck.length > 0) {
        try {
          const videos = await kickPlayback.getVideos(params.channelName, { limit: 50 });
...
export function registerVideoHandlers({ readers }: VideoHandlerDependencies): void {
  ipcMain.handle(
    IPC_CHANNELS.CLIPS_GET_BY_CATEGORY,
    async (_event, request: CategoryClipsRequest): Promise<CategoryMediaResult> => {
      try {
        const result = await readers[request.platform].readCategoryClips(
          {
          ...
          },
          {
            ...
            sort: request.sort === "date" ? "recent" : "popular",
            direction: request.direction === "asc" ? "ascending" : "descending",
            language: request.language,
            timeRange: request.timeRange,
          }
        );
        return toCategoryMediaResult(result, request);
      } catch (error) {
        return {
          success: false,
          availability: "unavailable",
          ...
          error:
            error instanceof Error
              ? error.message
              : `Failed to fetch ${request.platform} Category Clips`,
        };
      }
    }
  );
  ...
  ipcMain.handle(
    ...
    async (_event, request: CategoryVideosRequest): Promise<CategoryMediaResult> => {
      try {
        const result = await readers[request.platform].readCategoryVideos(
          {
            id: request.categoryId,
            name: request.categoryName,
            slug: request.categorySlug,
        ...
        return {
          ...
          error:
            error instanceof Error
              ? error.message
              : `Failed to fetch ${request.platform} Category Videos`,
        };
      }
    }
  );
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const { twitchPlayback } =
        await import("@backend/features/playback/composition/twitch-playback");

      try {
        if (params.platform === "twitch") {
          const video = await twitchPlayback.getVideoById(params.videoId);
          if (!video) {
    ...
    async (
      ...
      try {
        if (params.platform === "twitch") {
          ...
          const videos = await twitchPlayback.getVideosByChannel(channelLogin, {
            first: params.limit,
            after: params.cursor,
          });
          ...
          if (videoIds.length > 0) {
            try {
              gameMap = await twitchPlayback.getVideosGameData(videoIds);
            } catch (err) {
          ...
          const videos = await kickPlayback.getVideos(params.channelName, {
            limit: params.limit,
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const { kickPlayback } =
          await import("@backend/features/playback/composition/kick-playback");
        ...
        while (attempts < maxAttempts) {
          attempts++;
          const videos = await kickPlayback.getVideos(params.channelSlug, {
            limit: 50,
            cursor: cursor,
          });
