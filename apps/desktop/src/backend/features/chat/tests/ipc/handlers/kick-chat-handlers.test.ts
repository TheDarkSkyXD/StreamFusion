...
vi.mock("@backend/features/moderation/adapters/kick/kick-mod-mutations", () => ({
  banKickUserOfficial: vi.fn(),
  timeoutKickUserOfficial: vi.fn(),
  unbanKickUserOfficial: vi.fn(),
  setKickChatMode: vi.fn(),
}));
...
import {
  ...
  timeoutKickChatUser,
  unbanKickChatUser,
} from "@backend/features/chat/adapters/kick/kick-send-window";
import type {
  KickChannelViewerRoleResult,
  KickSendResult,
  KickWebApiMutationResult,
} from "@backend/features/chat/adapters/kick/kick-send-window";
import { registerKickChatHandlers } from "@backend/features/chat/routes/kick-chat-routes";
...
describe("registerKickChatHandlers", () => {
  it("registers Kick chat IPC channels", () => {
    ...
    expect(channels).toContain(IPC_CHANNELS.KICK_CHAT_MODERATE_BAN);
    expect(channels).toContain(IPC_CHANNELS.KICK_CHAT_MODERATE_TIMEOUT);
    expect(channels).toContain(IPC_CHANNELS.KICK_CHAT_MODERATE_UNBAN);
    expect(channels).toContain(IPC_CHANNELS.KICK_CHAT_SET_MODE);
...
describe("trusted Kick moderation bridge", () => {
  const trustedEvent = { senderFrame: { url: "http://localhost:5173/#/stream/kick/xqc" } };

  beforeEach(() => {
    vi.mocked(kickAuthService.ensureValidToken).mockResolvedValue(true);
    vi.mocked(kickAuthService.getAccessToken).mockReturnValue("main-only-token");
  });

  it("keeps the credential in main while forwarding validated official IDs", async () => {
    vi.mocked(banKickUserOfficial).mockResolvedValue({ ok: true });
    const handler = getHandler(IPC_CHANNELS.KICK_CHAT_MODERATE_BAN);

    await expect(handler(trustedEvent, { broadcasterUserId: 12, userId: 34, reason: "spam" })).resolves.toEqual({
      ok: true,
    });

    expect(kickAuthService.ensureValidToken).toHaveBeenCalledOnce();
    expect(banKickUserOfficial).toHaveBeenCalledWith({
      broadcasterUserId: 12,
      userId: 34,
      reason: "spam",
  ...
  it("rejects malformed mode changes before reading an auth credential", async () => {
    const handler = getHandler(IPC_CHANNELS.KICK_CHAT_SET_MODE);

    await expect(handler(trustedEvent, { channelSlug: "xqc", update: { slowMode: { seconds: 5 } } })).resolves.toMatchObject({
      ok: false,
      kind: "unknown",
    });
    expect(kickAuthService.ensureValidToken).not.toHaveBeenCalled();
    expect(setKickChatMode).not.toHaveBeenCalled();
  });
});
