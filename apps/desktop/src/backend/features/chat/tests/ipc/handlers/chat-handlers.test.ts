...
vi.mock("@backend/features/chat/adapters/kick/chat-endpoints", () => ({
  getKickChannelHistory: vi.fn(),
}));

vi.mock("@backend/features/chat/adapters/twitch/chat-endpoints", () => ({
...
vi.mock("@backend/features/discovery/composition/twitch-discovery", () => ({
  twitchDiscovery: { getUsersByLogin: vi.fn(), getChannelByLogin: vi.fn() },
}));
...
import { registerChatHandlers } from "@backend/features/chat/routes/chat-routes";

type MentionResult = {
  success: boolean;
  data: Array<{ userId: string; username: string; displayName: string; avatarUrl: string }>;
  error?: string;
};
...
beforeEach(() => {
  vi.clearAllMocks();
  twitchBadgeCatalogProvider.clearCache();
...
describe("CHAT_GET_KICK_HISTORY", () => {
  it("returns { success: true, data } on success", async () => {
    const { getKickChannelHistory } =
      await import("@backend/features/chat/adapters/kick/chat-endpoints");
...
describe("CHAT_GET_TWITCH_HISTORY", () => {
  it("returns { success: true, data } on success", async () => {
    const { getTwitchChannelHistory } =
      await import("@backend/features/chat/adapters/twitch/chat-endpoints");
...
describe("CHAT_ENRICH_MENTION_USERS", () => {
  it("returns a Kick default avatar when a channel user profile has no custom image", async () => {
    vi.mocked(kickDiscovery.getUsersById).mockResolvedValue([]);
    vi.mocked(kickDiscovery.getPublicChannelUserProfile).mockResolvedValue({
      userId: "4357508",
  ...
  it("returns fallback avatars for remaining known Kick chatters after all lookups miss", async () => {
    vi.mocked(kickDiscovery.getUsersById).mockResolvedValue([]);
    vi.mocked(kickDiscovery.getPublicChannelUserProfile).mockResolvedValue(null);
    vi.mocked(kickDiscovery.getPublicChannel).mockResolvedValue(null);
