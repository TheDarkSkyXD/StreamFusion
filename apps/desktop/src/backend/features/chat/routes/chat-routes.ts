...
import { twitchBadgeCatalogProvider } from "@backend/features/chat/adapters/twitch/twitch-badge-catalog-provider";
...
export function registerChatHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (_event, params: { channelId: string; channelSlug: string }) => {
      try {
        const { getKickChannelHistory } = await import("../adapters/kick/chat-endpoints");
  ...
  ipcMain.handle(
    ...
    async (_event, params: { channel: string }) => {
      try {
        const { getTwitchChannelHistory } = await import("../adapters/twitch/chat-endpoints");
  ...
  ipcMain.handle(IPC_CHANNELS.CHAT_GET_TWITCH_BADGE_CATALOG, async (_event, params: unknown) => {
    ...
    try {
      const userToken = authenticationRepository.isTokenExpired("twitch")
        ? null
        : authenticationRepository.getToken("twitch");
      const appToken = authenticationRepository.isAppTokenExpired("twitch")
        ? null
        : authenticationRepository.getAppToken("twitch");
      const accessToken = userToken?.accessToken || appToken?.accessToken || "";
      const catalog = await twitchBadgeCatalogProvider.loadBadgeCatalog(
        broadcasterId,
        channelLogin,
        { accessToken, clientId: TWITCH_APP_CLIENT_ID },
        {
          forceRefresh:
      ...
      try {
        ...
        if (params.platform === "twitch") {
          ...
          try {
            for (const batch of chunk(usersToFetch, 100)) {
              const fetched = await twitchDiscovery.getUsersByLogin(
          ...
          for (const user of usersToFetch) {
            ...
            try {
              const channel = await twitchDiscovery.getChannelByLogin(user.username);
              if (!channel) continue;
              const enriched = {
                userId: channel.id,
        ...
        if (ids.length > 0) {
          ...
          try {
            for (const batch of chunk(ids, 100)) {
              const fetched = await kickDiscovery.getUsersById(batch);
              for (const user of fetched) {
                const enriched = {
        ...
        if (channelSlug) {
          for (const user of usersToFetch) {
            if (found.has(user.username.toLowerCase())) continue;
            try {
              const profile = await kickDiscovery.getPublicChannelUserProfile(
                channelSlug,
                user.username
              );
              if (!profile) continue;
        ...
        for (const user of usersToFetch) {
          if (found.has(user.username.toLowerCase())) continue;
          try {
            const channel = await kickDiscovery.getPublicChannel(user.username);
