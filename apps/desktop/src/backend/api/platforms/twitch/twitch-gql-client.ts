 ...
 */
async function gqlRequest<T extends readonly any[]>(
    queries: [...T]
): Promise<any[]> {
...
function transformGqlStream(
    ...
    const thumbnailUrl = stream.previewImageURL
        .replace("{width}", "440")
        .replace("{height}", "248");
...
export async function gqlGetTopStreams(
    ...
    if (options.gameId) {
        ...
        const streams = game.streams.edges.map((edge) =>
            transformGqlStream(edge.node)
        );
  ...
  `;
    ...
    if (response.errors) {
        console.warn("⚠️ [GQL] TopStreams query errors:", response.errors.map((e: any) => e.message).join(", "));
    }

    const data = response.data;
    if (!data?.streams) return { data: [] };

    const streams = data.streams.edges.map((edge) =>
        transformGqlStream(edge.node)
    );
 ...
 */
export async function gqlGetStreamByLogin(
    login: string
 ...
 */
export async function gqlGetStreamsByLogins(
    logins: string[]
): Promise<UnifiedStream[]> {
    if (logins.length === 0) return [];

    // Batch UseLive queries to check which channels are live
    const queries = logins.map((login) =>
        getQueryUseLive({ channelLogin: login })
    );
 ...
 */
export async function gqlGetChannelByLogin(
    login: string
 ...
 */
export async function gqlGetChannelsByLogins(
    logins: string[]
...
export async function gqlGetClipsByChannel(
    ...
    const limit = options.first || 20;
    const filter = (options.filter as "LAST_DAY" | "LAST_WEEK" | "LAST_MONTH" | "ALL_TIME") || "LAST_WEEK";
 ...
 */
export async function gqlGetClipAccessToken(
    slug: string
): Promise<{
    ...
    const [response] = await gqlRequest([
        getQueryVideoAccessTokenClip({ slug }),
