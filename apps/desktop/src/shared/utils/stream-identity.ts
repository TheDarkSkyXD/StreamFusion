...
import type { UnifiedChannel, UnifiedStream } from "../platform-types";
...
function mergeDuplicateChannel(existing: UnifiedChannel, incoming: UnifiedChannel): UnifiedChannel {
  const primary = channelMetadataScore(incoming) > channelMetadataScore(existing) ? incoming : existing;
...
/** Collapses channel rows by platform-scoped canonical identity. */
export function dedupeChannelsByIdentity(channels: UnifiedChannel[]): UnifiedChannel[] {
  ...
  for (const channel of channels) {
    ...
    if (existingIndex === -1) deduped.push(channel);
    else deduped[existingIndex] = mergeDuplicateChannel(deduped[existingIndex], channel);
  }
  return deduped;
}
  ...
  return {
    ...
    tags:
      Array.isArray(primary.tags) && primary.tags.length > 0
        ? primary.tags
        : Array.isArray(fallback.tags) && fallback.tags.length > 0
          ? fallback.tags
          : [],
...
/** Collapses live results by platform-scoped broadcaster identity. */
