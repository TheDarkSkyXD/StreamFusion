---
name: superdev-mode
description: SuperDev's agent style for concise, detailed responses, deliberate subagents, unslopped prose, simple code, and verified work. Use for SuperDev, /superdev-mode, or requests to work in this style.
---
...
- About to use the harness's user-input tool on a "which approach", "how should I", or "what should this do" fork → classify it before you ask. If the answer is a fact you could observe by running something (behavior, timing, layout, output, perf, even whether an eval separates), it is not the human's to answer. Sketch it via the Prototype playbook (`playbooks/prototype.md`) and let the result decide. If the task is a read-only Investigation whose deliverable is a cited answer, stay in it and answer from the evidence rather than building a sketch. Reserve the question for a genuine product or preference call no experiment can settle. The ask is the slow path. A throwaway probe usually answers faster, and it hands the human a result to react to instead of a decision to make.
- Any code → name the data shape first, and choose its organizing structure per **principle-model-the-domain**.
- Code crossing a function boundary → the **architect** skill, parallel design exploration before implementing.
- Parallel fan-out → the **swarm** skill for coverage matrices, races, gauntlets, and exploration partitions. Use **arena** for design or code bakeoffs with base selection and grafting.
- Contested design → the **interrogate** skill (multi-model adversarial) before shipping.
- Nontrivial multi-step → write the throughput checkpoint (Feature step 3).
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the active harness's skill-authoring guidance.
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill (`/technical-writing`).
- Before commit → run the **deslop** skill (`/deslop`) against the diff. If it is unavailable, inspect the diff directly for narration, unsupported guards, dead compatibility paths, and unrelated edits.
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → use the active harness's browser, desktop, terminal, or service-control capability that matches the surface. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
...
**Use the `superdev-agent` definition at `.agents/agents/superdev-agent.md` for any subagent you spawn inside a playbook step** (code-writing delegates and ad-hoc helpers). Invoke it through the active harness's delegation interface. `/superdev-mode` and `superdev-agent` route through the same wrapper. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) prescribe their own worker shape for diverse-model review; respect those instructions.

**Defaults for every delegated task.** Run independent work in the background when the harness supports it, use file pointers instead of inlining large context, and select models by role from `.agents/superdev-models.md`. If the file or a role is absent, use the strongest available judgment model for vague or cross-cutting work, the strongest instruction-following model for precise sequences, and a fast model for mechanical edits. A value of `inherit-parent` or `auto` means to use the parent chat model.
...
- **Refactoring.** A behavior-preserving change to structure or clarity (refactor, simplify, readability pass, reduce complexity, rename, extract, inline, dedupe, move). `playbooks/refactoring.md`.
