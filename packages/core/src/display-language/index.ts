...
export function isSupportedDisplayLanguageInput(value: unknown): boolean {
  ...
  if (DISPLAY_LANGUAGE_BY_NORMALIZED_CODE.has(normalized)) return true;
  if (DISPLAY_LANGUAGE_ALIASES.has(normalized)) return true;
  if (
  ...
  return DISPLAY_LANGUAGE_BY_NORMALIZED_CODE.has(base);
}

export function resolveDisplayLanguage(value: unknown): DisplayLanguage {
  if (typeof value !== "string") return DEFAULT_DISPLAY_LANGUAGE;
  const normalized = value.trim().toLowerCase();
  if (!normalized) return DEFAULT_DISPLAY_LANGUAGE;

  const exact = DISPLAY_LANGUAGE_BY_NORMALIZED_CODE.get(normalized);
