...
const outDir = path.resolve("verification/evidence/issue-165");
...
function waitFor(predicate, label, attempts = 24, swipe = false, delayMs = 1200) {
  let xml = "";
...
startFixture(
  "media-jobs-start-compressed-recording",
  "Start compressed four-hour cutoff",
);
waitFor(
  (node) => node.text === "Recording will stop at the four-hour limit.",
  "four-hour warning",
  40,
  false,
  350,
);
shots.push(screenshot("02-warning"));
waitFor(
  (node) =>
    node.text === "Stopped at the four-hour limit. Partial recording saved.",
  "compressed cutoff completed",
  24,
  false,
  350,
);
shots.push(screenshot("03-cutoff-complete"));
const openControl =
  findControl(dumpUi(), "media-job-open", "Open") ??
  findNode(dumpUi(), (node) => node.clickable && node.desc === "Open");
if (openControl) {
  tap(openControl);
  sleep(2000);
  shots.push(screenshot("04-open-partial"));
  run(["shell", "input", "keyevent", "4"]);
  sleep(1000);
}
leaveJobPreview();
startFixture("media-jobs-start-recording", "Start fixture recording");
const stopWait = waitFor(
  (node) =>
    node.res === "media-job-command-finalize" ||
    node.desc === "Stop" ||
    node.text === "Stop",
  "Stop command",
  20,
...
shots.push(screenshot("05-recording-running"));
tap(stopWait.node);
sleep(1500);
waitFor(
  (node) =>
    node.text === "Completed" ||
    node.text.includes("Partial recording saved") ||
    node.text === "Stopped. Partial recording saved.",
  "stopped partial recording",
  20,
  true,
);
shots.push(screenshot("06-partial-artifact"));
leaveJobPreview();
startFixture(
  "media-jobs-start-recording-storage-pressure",
...
waitFor(
  (node) =>
    ...
    node.text.includes("Failed"),
  "recording storage-pressure failure",
  24,
);
shots.push(screenshot("07-storage-pressure"));
startFixture(
  "media-jobs-start-compressed-recording",
  "Start compressed four-hour cutoff",
);
...
waitFor(
  ...
  "recovered recording after process death",
  16,
  true,
);
...
waitFor(
  (node) =>
    ...
    node.text.includes("Completed") ||
    node.text.includes("Failed"),
  "recovered recording after reboot",
  20,
  true,
);
shots.push(screenshot("09-reboot"));
